...
def crear_venta_desde_inventario(request, pk):
    ...
    return redirect(_desarme_url(request, f"vehiculos/{vehiculo.pk}/confirmar-venta-desde-inventario/"))
...
def _get_venta_session_data(request, vehiculo):
    ...
    piezas_qs = PiezaDesarme.objects.filter(vehiculo=vehiculo, pk__in=ids).prefetch_related("names", "company_labels")
    ...
    for row in items:
        ...
        display_nombre = pieza.get_display_label(empresa=empresa, language=lang) if empresa else (pieza.nombre or "")
