# 🚀 Cómo Ejecutar el Reporte de Suscripciones

## ⚠️ Problema Actual

Hay un error de configuración con allauth que impide ejecutar comandos de Django directamente. La solución es usar el **shell de Django**.

## ✅ Solución: Usar el Shell de Django
...
for pais_codigo in paises_todos:
    ...
    print("-" * 80)
    empresas = Empresa.objects.filter(pais=pais_codigo, suscripcion_activa=True).select_related("user")

    if empresas.exists():
        for idx, emp in enumerate(empresas, 1):
            vencida = timezone.now() > emp.fecha_expiracion if hasattr(emp, "fecha_expiracion") else False
            estado = "❌ VENCIDA" if vencida else "✅ ACTIVA"
            ...
            print(f"     Días Restantes: {emp.dias_restantes if hasattr(emp, 'dias_restantes') else 'N/A'}")
...
## 🔍 Consultas Rápidas (Solo números)

Si solo necesitas números rápidos, ejecuta esto en el shell:
...
# Totales
print("Empresas activas:", Empresa.objects.filter(suscripcion_activa=True).count())
print("Suscripciones activas:", Suscripcion.objects.filter(activa=True).count())
print("Trials activos:", TrialRegistro.objects.filter(prueba_activa=True).count())

# Por país
for pais in ["CL", "US", "MX"]:
