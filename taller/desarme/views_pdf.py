...
def _document_filename(documento):
    base = getattr(documento, "numero_documento", None) or getattr(documento, "numero", None) or f"DOC-{documento.id}"
...
def _get_documento_queryset():
    return Documento.objects.select_related("empresa", "cliente", "vehiculo").prefetch_related("lineas_repuesto")
...
def compartir_documento_whatsapp(request, documento_id):
    ...
    numero = getattr(documento, "numero_documento", None) or getattr(documento, "numero", None) or f"#{documento.id}"
    mensaje = (
