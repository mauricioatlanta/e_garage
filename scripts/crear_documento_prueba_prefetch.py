...
def crear_documento_prueba():
    ...
    print(f"🔧 Repuesto 1: {linea_rep1.nombre} - ${linea_rep1.precio_unitario} x {linea_rep1.cantidad}")
    ...
    print(f"🔧 Repuesto 2: {linea_rep2.nombre} - ${linea_rep2.precio_unitario} x {linea_rep2.cantidad}")
...
def probar_vista_ver_documento(documento):
    ...
    documento_optimizado = Documento.objects.select_related(
    ...
    for rep in repuestos:
        ...
        print(f"   - {rep.nombre}: ${precio_rep} x {cantidad_rep} (desc: {descuento_rep}%) = ${subtotal}")
    ...
    subtotal_servicios = sum(
        serv.precio_unitario * (1 - serv.descuento / 100)
        for serv in servicios
    )
...
if __name__ == '__main__':
    try:
        ...
        if documento:
            ...
            print(f"🔗 Puede probarlo en la interfaz web navegando a ver documento #{documento.numero_documento}")
        else:
