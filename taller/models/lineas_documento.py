...
class LineaRepuesto(models.Model):
    ...
    def clean(self):
        ...
        if self.origen_repuesto == ORIGEN_DESARME:
            ...
            if hasattr(self, "documento") and self.documento_id and self.pieza_desarme.empresa_id != self.documento.empresa_id:
                raise ValidationError(
                    "La pieza de desarme debe pertenecer a la misma empresa del documento."
                )
            if not getattr(self.pieza_desarme, "activo", True):
                raise ValidationError(
                    "La pieza de desarme no está disponible (inactiva)."
                )
            ...
            if not (self.repuesto_id or self.part_id):
                raise ValidationError(
                    "Origen Stock bodega requiere repuesto o part."
                )
            if hasattr(self, "documento") and self.documento_id:
                ...
                if doc_empresa_id is not None:
                    if self.repuesto_id and getattr(self.repuesto, "empresa_id", None) != doc_empresa_id:
                        raise ValidationError(
                            "El repuesto debe pertenecer a la misma empresa del documento."
                        )
                    if self.part_id and getattr(self.part, "empresa_id", None) is not None and self.part.empresa_id != doc_empresa_id:
                        raise ValidationError(
