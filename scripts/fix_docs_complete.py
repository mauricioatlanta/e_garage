        ...
        try:
            ...
            if servicio_ext:
                ...
                doc.total = (
                    ...
                    + (doc.neto_servicios or Decimal("0"))
                    + doc.neto_otros_servicios
                )
                doc.save()
...
for doc in Documento.objects.filter(empresa=empresa).order_by("id"):
    ...
    print(
        f"Doc {doc.pk} ({doc.tipo}): MILLAS={millas}, #REP={rep_count}, #SERV={serv_count}, #OTROS={otros_count}, TOTAL=${doc.total}"
    )
