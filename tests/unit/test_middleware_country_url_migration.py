...
def test_country_url_migration_cases():
    ...
    try:
        from taller.middleware.country_url_migration import CountryUrlMigrationMiddleware
    except ImportError:
    ...
    test_cases = [
        ...
        ("/documentos/?filter=active&sort=name", "/cl/documentos/?filter=active&sort=name", "Multiple query params"),
        ("/documentos", "/cl/documentos/", "Path without trailing slash"),
        ("/DOCUMENTOS/", "/cl/documentos/", "Uppercase path"),
        ("/documentos/?NEXT=/other", "/cl/documentos/?NEXT=/other", "Preserve uppercase query params"),
        ("/documentos/?next=/other", "/cl/documentos/?next=/other", "Preserve lowercase query params"),
        ("/documentos/?next=/cl/other", "/cl/documentos/?next=/cl/other", "Next param with existing prefix"),
    ]
    ...
    for input_url, expected_redirect, description in test_cases:
        ...
        if response and hasattr(response, 'status_code'):
            assert response.status_code in (301, 302), f"Failed for {description}: {input_url}"
        else:
            # If no redirect, the URL should be processed
            assert request.path.startswith('/cl/') or request.path == input_url, f"Failed for {description}: {input_url}"
    ...
    for url, expected_country, description in test_cases:
        ...
        if hasattr(request, 'country'):
            ...
            if description == "No prefix" and expected_country is None:
                assert request.country in (None, "CL"), f"Failed for {description}: expected None or CL, got {request.country}"
            else:
                assert request.country == expected_country, f"Failed for {description}: expected {expected_country}, got {request.country}"
