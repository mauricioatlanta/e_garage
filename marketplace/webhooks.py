...
def webhook_whatsapp_cliente(request):
    ...
    try:
        ...
        if not _verificar_token_webhook(request, provider):
            logger.warning(
                f"Intento de webhook sin token válido desde {request.META.get('REMOTE_ADDR')}"
            )
        ...
        if resultado == "APROBADO":
            ...
            return JsonResponse(
                {"success": True, "action": "documento_rechazado", "message": "Documento rechazado"}
            )
        else:
            return JsonResponse(
                {"success": False, "message": "Respuesta no reconocida"}, status=400
            )
...
def webhook_whatsapp_proveedor(request):
    ...
    try:
        ...
        if not _verificar_token_webhook(request, provider):
            logger.warning(
                f"Intento de webhook proveedor sin token válido desde {request.META.get('REMOTE_ADDR')}"
            )
            return JsonResponse({"error": "Token inválido"}, status=403)
        ...
        if any(
            ...
            for palabra in ["confirmo", "disponible", "si", "sí", "ok", "stock"]
        ):
            ...
            return JsonResponse(
                {"success": False, "message": "Confirmación no reconocida"}, status=400
            )
