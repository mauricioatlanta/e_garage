...
def show_urls(urlconf, prefix='', indent=0):
    ...
    for pattern in resolver.url_patterns:
        if hasattr(pattern, 'url_patterns'):
            ...
            if 'ingreso' in callback_str.lower() or 'patente' in callback_str.lower() or 'ingreso' in str(pattern.pattern):
                print(' ' * indent + f">>> {prefix}{pattern.pattern} -> {callback_str} [name: {url_name}]")
