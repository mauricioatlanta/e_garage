    ...
    for path, country, must_contain, must_not_contain in paths:
        ...
        if resp.status_code == 200:
            if has_expected and not has_forbidden:
                print(f"OK {country} {path}: contiene '{must_contain}', no contiene '{must_not_contain}'")
                ok += 1
            else:
                print(f"FAIL {country} {path}: expected '{must_contain}' in HTML, forbidden '{must_not_contain}'")
                print(f"  status={resp.status_code} has_expected={has_expected} has_forbidden={has_forbidden}")
        else:
            # Redirect a login u otro: igualmente comprobar que no haya mezcla en el body
            if has_forbidden:
                print(f"WARN {country} {path}: status {resp.status_code} pero HTML contiene '{must_not_contain}'")
            else:
