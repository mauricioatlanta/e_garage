...
from taller.models import Documento, Repuesto, LineaRepuesto, LineaServicio, LineaOtroServicio
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **opts):
        ...
        for doc in qs.iterator():
            ...
            detalles_raw = (
                getattr(doc, "detalles", None) or 
                getattr(doc, "json_data", None) or
                getattr(doc, "data_payload", None) or
                getattr(doc, "items_data", None)
            )
            ...
            if not detalles_raw:
                ...
                try:
                    ...
                    if detalles_count > 0:
                        self.stdout.write(self.style.NOTICE(f"[INFO] Doc {doc.id} tiene {detalles_count} DetalleDocumento pero no JSON detalles"))
                except:
                    pass
                    
                self.stdout.write(self.style.WARNING(f"[SKIP] Doc {doc.id} sin datos de 'detalles' o campos relacionados"))
                continue

            try:
                detalles = detalles_raw if isinstance(detalles_raw, dict) else json.loads(detalles_raw)
            ...
            if opts["dry_run"]:
                self.stdout.write(self.style.NOTICE(f"[DRY] Doc {doc.id} crear rep:{len(repuestos)} ser:{len(servicios)} otr:{len(otros)}"))
                procesados += 1
            ...
            procesados += 1
            creadas += (len(repuestos)+len(servicios)+len(otros))
            self.stdout.write(self.style.SUCCESS(f"[OK] Doc {doc.id} líneas creadas: rep:{len(repuestos)} ser:{len(servicios)} otr:{len(otros)}"))

        self.stdout.write(self.style.SUCCESS(f"Procesados: {procesados}, líneas creadas: {creadas}"))
