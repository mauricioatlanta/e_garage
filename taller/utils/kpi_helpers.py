...
from django.db.models import F, ExpressionWrapper, DecimalField, Sum, Count, Avg, Q
...
class KPICalculator:
    ...
    def get_rendimiento_mensual(self, meses=12):
        ...
        return (LineaServicio.objects
                .filter(filtro)
                .annotate(
                    mes=TruncMonth("documento__fecha_emision"),
                    monto=self.monto_calculado
                )
                .values("mes")
                .annotate(
                    total=Sum("monto"),
    ...
    def get_rendimiento_anual(self, anos=3):
        ...
        return (LineaServicio.objects
                .filter(filtro)
                .annotate(
                    ano=TruncYear("documento__fecha_emision"),
                    monto=self.monto_calculado
                )
                .values("ano")
                .annotate(
