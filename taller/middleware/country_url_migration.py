...
class CountryURLRedirectMiddleware(MiddlewareMixin):
        ...
        for old_prefix, new_prefix in self.URL_MIGRATIONS.items():
            ...
            if re.match(pattern, path):
                ...
                if hasattr(request, 'user') and request.user.is_authenticated:
                    print(f"🔄 URL Migration: {request.path} → {new_url} (User: {request.user.username})")
                else:
    ...
    def process_response(self, request, response):
        ...
        if hasattr(request, '_needs_country_redirect'):
            ...
            response['X-Migration-Info'] = 'Legacy URL prefix detected, please update to new country-based URLs'
