...
from taller.models import LineaServicio, Documento, Tecnico
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            for total in totales:
                self.stdout.write(f"   - {total['documento__tecnico_responsable__nombre']}: ${total['total']}")
            
            # KPI 2: Documentos por estado en el mes actual
            self.stdout.write("\n2. Documentos por estado en el mes actual:")
            docs_por_estado = (Documento.objects
            ...
            tecnicos_activos = (Tecnico.objects
                .filter(documento__fecha_emision__month=timezone.now().month)
                .values("nombre")
            ...
            self.stdout.write(
                self.style.ERROR(f"❌ Error en KPI sanity check: {e}")
            )
            raise
