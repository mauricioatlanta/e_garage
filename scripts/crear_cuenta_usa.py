...
from taller.models.documento import (Documento, OtroServicioDocumento,
                                     RepuestoDocumento, ServicioDocumento)
...
def crear_cuenta_usa_completa():
    ...
    print(f"  - Total parts in docs: {RepuestoDocumento.objects.filter(documento__empresa=empresa_usa).count()}")
    print(f"  - Total services in docs: {ServicioDocumento.objects.filter(empresa=empresa_usa).count()}")
    print(f"  - Total other services: {OtroServicioDocumento.objects.filter(documento__empresa=empresa_usa).count()}")
