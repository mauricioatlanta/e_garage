...
for i, doc in enumerate(docs):
    ...
    if servicios_existentes.count() == 0:
        ...
        precio = getattr(
            servicio,
            "precio_cliente",
            getattr(servicio, "precio_base", Decimal("50.00")),
        )
        ...
        try:
            linea_servicio = LineaServicio.objects.create(
                ...
                descuento=Decimal("0"),
                observaciones="Servicio agregado automáticamente",
            )
            print(
                f"✅ Servicio agregado: {linea_servicio.nombre} - ${linea_servicio.precio_unitario}"
            )
            ...
            doc.total = (
                (doc.neto_repuestos or Decimal("0"))
                + doc.neto_servicios
                + (doc.neto_otros_servicios or Decimal("0"))
            )
            doc.save()
