...
def test_eliminar_documento():
    ...
    for doc in documentos:
        print(
            f"  - ID: {doc.id}, Tipo: {doc.tipo_documento}, Número: {doc.numero_documento}, Empresa: {doc.empresa}"
        )
    ...
    for empresa in empresas:
        print(
            f"  - ID: {empresa.id}, Nombre: {empresa.nombre if hasattr(empresa, 'nombre') else 'Sin nombre'}"
        )
...
def crear_datos_prueba():
    ...
    empresa, created = Empresa.objects.get_or_create(nombre="Taller de Prueba")
    if created:
