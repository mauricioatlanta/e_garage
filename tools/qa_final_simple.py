...
from taller.models import Empresa, Cliente, Vehiculo, Tecnico, Documento, LineaRepuesto, LineaServicio, LineaOtroServicio, Marca
...
try:
    ...
    if not cliente_cl or not vehiculo_cl or not tecnico_cl:
        ...
        if doc_cl.total_general == expected_total:
            ...
            print(f"ERROR: Calculos CL incorrectos: esperado {expected_total}, obtenido {doc_cl.total_general}")
...
try:
    ...
    if not cliente_us or not vehiculo_us or not tecnico_us:
        print("ERROR: Faltan datos basicos para USA")
    else:
        # Crear documento
        ...
        print(f"OK: Documento US creado: ID {doc_us.id}")
        ...
        if doc_us.total_general == expected_total:
            print("OK: Calculos US correctos")
        else:
            print(f"ERROR: Calculos US incorrectos: esperado {expected_total}, obtenido {doc_us.total_general}")
