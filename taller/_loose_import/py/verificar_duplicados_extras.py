...
def analizar_duplicados_colores():
    ...
    if hasattr(ColorVehiculo, "_meta") and "country" in [
        f.name for f in ColorVehiculo._meta.get_fields()
    ]:
        duplicados = (
            ColorVehiculo.objects.values("country")
        ...
        duplicados = (
            ColorVehiculo.objects.annotate(lower_nombre=Lower("nombre"))
            .values("lower_nombre")
            .annotate(count=Count("id"))
            .filter(count__gt=1)
        )
        ...
        for dup in duplicados:
            ...
            if country != "N/A":
                registros = ColorVehiculo.objects.filter(
                    nombre__iexact=lower_nombre, country=country
                )
            else:
...
def analizar_duplicados_motores():
    ...
    if hasattr(MotorVehiculo, "_meta") and "country" in [
        f.name for f in MotorVehiculo._meta.get_fields()
    ]:
        duplicados = (
            MotorVehiculo.objects.values("country")
            .annotate(lower_nombre=Lower("nombre"))
            .values("country", "lower_nombre")
            .annotate(count=Count("id"))
            .filter(count__gt=1)
        ...
        duplicados = (
            MotorVehiculo.objects.annotate(lower_nombre=Lower("nombre"))
            .values("lower_nombre")
            .annotate(count=Count("id"))
            .filter(count__gt=1)
        )
    ...
    if duplicados:
        ...
        for dup in duplicados:
            ...
            for reg in registros:
                ...
                print(
                    f"     → ID {reg.id}: '{reg.nombre}' (usado en {usos} vehículos, {modelos_asociados} modelos)"
                )
...
def analizar_duplicados_cajas():
    ...
    if hasattr(CajaVehiculo, "_meta") and "country" in [
        f.name for f in CajaVehiculo._meta.get_fields()
    ]:
        duplicados = (
            CajaVehiculo.objects.values("country")
            .annotate(lower_nombre=Lower("nombre"))
            .values("country", "lower_nombre")
            .annotate(count=Count("id"))
            .filter(count__gt=1)
        ...
        duplicados = (
            CajaVehiculo.objects.annotate(lower_nombre=Lower("nombre"))
            .values("lower_nombre")
            .annotate(count=Count("id"))
            .filter(count__gt=1)
        )
