    ...
    try:
        ...
        if changes_made > 0:
            if not dry_run:
                ...
                print(f"   ✅ {file_path} - {changes_made} referencias actualizadas (backup: {backup_path.name})")
            else:
...
def update_template_references(templates_dir, static_dir, manifest_file, dry_run=True):
    ...
    print(f"🔍 Actualizando referencias...")
    ...
    if dry_run:
        ...
        print(f"   python update_template_refs.py --templates {templates_path} --static {static_path} --manifest {manifest_file}")
    else:
        print(f"\n✅ Cambios aplicados exitosamente")
        print(f"   Los archivos originales se guardaron con extensión .bak")
