...
def _build_usa_catalog_indexes():
    ...
    for p in CATALOGO_PIEZAS_DESARME_USA:
        ...
        if not codigo:
            continue
        nombre_en = (p.get("nombre_en_oficial") or p.get("nombre_en_slang") or p.get("codigo") or "").strip()
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("--dry-run", action="store_true", help="Solo reportar, sin guardar cambios.")
        parser.add_argument("--batch", type=int, default=5000, help="Procesar en lotes de N piezas.")
                ...
                with transaction.atomic():
                    for obj in to_create_es:
                        try:
                            ...
                            conflicts += 1
                            if conflicts <= 5:
                                self.stdout.write(self.style.ERROR(f"  Conflicto es: pieza_id={obj.pieza_desarme_id} {e}"))
                    for obj in to_create_en:
                        try:
                            ...
                            conflicts += 1
                            if conflicts <= 5:
                                self.stdout.write(self.style.ERROR(f"  Conflicto en: pieza_id={obj.pieza_desarme_id} {e}"))
            else:
