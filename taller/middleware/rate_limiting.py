...
class RateLimiter:
    ...
    def __init__(self):
        ...
        self.limits = {
            ...
            "registration": {"attempts": 3, "window": 3600, "block_time": 1800},
            "api_call": {
                "attempts": 100,
                "window": 60,  # 1 minuto
    ...
    def check_rate_limit(self, action, identifier):
        ...
        attempts_data = cache.get(
            cache_key, {"count": 0, "reset_time": time.time() + config["window"]}
        )
    ...
    def decorator(view_func):
        ...
        def wrapper(request, *args, **kwargs):
            ...
            if any(suspicious_indicators):
                ...
                custom_config = {"attempts": 2, "window": 600, "block_time": 3600}
