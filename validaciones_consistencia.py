...
class ValidadorConsistencia:
    ...
    def validar_3_servicios_country_consistency(self):
        ...
        servicios_inconsistentes = Servicio.objects.select_related(
            ...
            country=F('subcategoria__country')
        )
    ...
    def validar_4_traducciones_completas(self):
        ...
        servicios_sin_es = Servicio.objects.exclude(
            names__language='es'
        ).distinct()
        ...
        duplicados = ServicioName.objects.values(
            'servicio', 'language'
        ).annotate(
            count=Count('id')
        ).filter(count__gt=1)
    ...
    def validar_5_unique_constraints(self):
        ...
        duplicados_servicio = Servicio.objects.values(
            'country', 'tipo', 'code'
        ).annotate(
            count=Count('id')
        ).filter(count__gt=1)
        ...
        duplicados_name = ServicioName.objects.filter(
            is_default=True
        ).values(
            'servicio', 'language'
        ).annotate(
    ...
    def validar_6_integridad_referencial(self):
        ...
        try:
            ...
            self.log_error(
                'VALIDATION_ERROR',
                f"Error validando integridad referencial: {str(e)}"
            )
    ...
    def validar_7_datos_por_pais(self):
        ...
        for pais in paises:
            ...
            if count_servicios == 0:
                self.log_advertencia(
                    'DATOS_FALTANTES',
                    f"País {pais} no tiene servicios",
                    Servicio
                )
    ...
    def ejecutar_validaciones_completas(self):
        ...
        try:
            ...
            self.log_error(
                'CRITICAL_ERROR',
                f"Error crítico en validaciones: {str(e)}"
            )
            traceback.print_exc()
