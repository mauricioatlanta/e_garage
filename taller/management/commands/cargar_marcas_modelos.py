...
class Command(BaseCommand):
    help = 'Carga marcas y modelos de vehículos desde diccionario predefinido'
    ...
    def handle(self, *args, **kwargs):
        ...
        for nombre_marca, modelos in marcas_y_modelos.items():
            marca_obj, creada = Marca.objects.get_or_create(nombre=nombre_marca)
            if creada:
                contador_marcas += 1
            for nombre_modelo in modelos:
                try:
                    ...
                    self.stdout.write(self.style.WARNING(f"Modelo duplicado ignorado: {nombre_modelo} ({nombre_marca})"))

        self.stdout.write(self.style.SUCCESS(f'✅ Creadas {contador_marcas} marcas y {contador_modelos} modelos.'))