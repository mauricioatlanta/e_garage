...
class WhatsAppFlowManager:
    ...
    def process_message(self, from_phone: str, message_type: str, content: Any) -> bool:
        ...
        if self.session.is_expired():
            ...
            self.meta_client.send_text_message(
                from_phone,
                "⏱ Tu sesión expiró. Escribe 'Nuevo' o '🆕' para comenzar."
            )
            return False
    ...
    def _handle_idle(self, from_phone: str, message_type: str, content: Any) -> bool:
        ...
        if message_type == 'text':
            ...
            if text_lower in ['nuevo', '🆕', 'nueva', 'nuevo vehiculo']:
                ...
                self.meta_client.send_text_message(
                    from_phone,
                    "❌ Operación cancelada. Escribe 'Nuevo' para comenzar."
                )
                return True
            else:
    ...
    def _handle_waiting_plate(self, from_phone: str, message_type: str, content: Any) -> bool:
        ...
        if message_type == 'image':
            ...
            if not media_id:
                self.meta_client.send_text_message(
                    from_phone,
                    "❌ No se pudo obtener la imagen. Intenta nuevamente."
                )
                return False
            ...
            if not image_bytes:
                self.meta_client.send_text_message(
                    from_phone,
                    "❌ Error descargando imagen. Intenta nuevamente."
                )
                return False
            ...
            self.meta_client.send_text_message(
                from_phone,
                "📸 Por favor, envía una foto de la patente."
            )
            return False
    ...
    def _handle_waiting_mileage(self, from_phone: str, message_type: str, content: Any) -> bool:
        ...
        if message_type == 'text':
            ...
            try:
                ...
                self.meta_client.send_text_message(
                    from_phone,
                    "❌ Por favor, envía un número válido de kilometraje (ej: 50000)."
                )
                return False
        else:
            self.meta_client.send_text_message(
                from_phone,
                "📊 Por favor, envía el kilometraje como número (ej: 50000)."
            )
            return False
    ...
    def _handle_waiting_action(self, from_phone: str, message_type: str, content: Any) -> bool:
        ...
        if message_type == 'interactive':
            ...
            if button_id == 'finalizar':
                ...
                self.meta_client.send_text_message(
                    from_phone,
                    "📸 Envía una foto o video como evidencia."
                )
            ...
            if not media_id:
                self.meta_client.send_text_message(
                    from_phone,
                    "❌ No se pudo obtener el audio. Intenta nuevamente."
                )
                return False
            ...
            if not audio_bytes:
                self.meta_client.send_text_message(
                    from_phone,
                    "❌ Error descargando audio. Intenta nuevamente."
                )
                return False
    ...
    def _handle_nlp_result(self, from_phone: str, result: Optional[Dict[str, Any]]) -> bool:
        ...
        if action == 'CREATE_OT':
            ...
            self.meta_client.send_text_message(
                from_phone,
                f"❌ Acción '{action}' no reconocida. Intenta nuevamente."
            )
            return False
    ...
    def _process_add_service(self, from_phone: str, data: Dict[str, Any]) -> bool:
        ...
        if not servicios:
            self.meta_client.send_text_message(
                from_phone,
                "❌ No se encontraron servicios en el mensaje."
            )
            return False
    ...
    def _process_add_part(self, from_phone: str, data: Dict[str, Any]) -> bool:
        ...
        if not repuestos:
            self.meta_client.send_text_message(
                from_phone,
                "❌ No se encontraron repuestos en el mensaje."
            )
            return False
    ...
    def _process_add_outsourced(self, from_phone: str, data: Dict[str, Any]) -> bool:
        ...
        if not externos:
            self.meta_client.send_text_message(
                from_phone,
                "❌ No se encontraron servicios externos en el mensaje."
            )
            return False
