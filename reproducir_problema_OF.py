...
def crear_documento_con_repuesto_OF():
    ...
    if not repuesto_of:
        ...
        print(f"✅ Creado repuesto OF: ID={repuesto_of.id}, part_number='{repuesto_of.part_number}'")
    else:
        print(f"✅ Repuesto OF existente: ID={repuesto_of.id}, part_number='{repuesto_of.part_number}'")
    ...
    if not all([cliente, tecnico, vehiculo]):
        print(f"❌ ERROR: Datos faltantes - cliente={bool(cliente)}, tecnico={bool(tecnico)}, vehiculo={bool(vehiculo)}")
        return False
    ...
    print(f"print('DOC:', doc.id, doc.empresa_id, doc.cliente_id, doc.vehiculo_id, getattr(doc, 'millas', None))")
    ...
    print(f"print('LIST rep ids:', list(doc.lineas_repuesto.values_list('id','repuesto_id','nombre','cantidad','precio_unitario','descuento')))")
    ...
    list_rep = list(doc.lineas_repuesto.values_list("id","repuesto_id","nombre","cantidad","precio_unitario","descuento"))
