...
class Repuesto(TenantScoped):
    ...
    categoria   = models.ForeignKey(CategoriaRepuesto, on_delete=models.PROTECT, db_index=True, null=True, blank=True)
    precio_compra = models.DecimalField(max_digits=10, decimal_places=2, default=Decimal('0.00'), help_text="Precio al que compraste el repuesto")
    precio_venta  = models.DecimalField(max_digits=10, decimal_places=2, default=Decimal('0.00'), help_text="Precio al que vendes el repuesto")
    cantidad_stock = models.PositiveIntegerField(default=0, help_text="Cantidad disponible en stock")
    proveedor = models.CharField(max_length=200, null=True, blank=True, help_text="Donde compraste el repuesto")
