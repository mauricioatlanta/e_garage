# apply_static_cleanup_clean.ps1
# Script limpio para limpieza de archivos estáticos en producción
param(
...
import-csv $auditCsv | ForEach-Object { $auditData += $_ }
...
if ($DryRun) {
  ...
  Write-Host "   .\apply_static_cleanup_clean.ps1 -Root `"$Root`"" -ForegroundColor White
} else {
