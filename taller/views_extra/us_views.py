...
def api_marcas_vehiculos_usa(request):
    ...
    marcas = MarcaVehiculo.objects.filter(pais_origen=pais, activa=True).values(
        "id", "nombre", "nombre_en", "pais_origen", "anio_inicio", "anio_fin"
    )

    return JsonResponse({"marcas": list(marcas), "total": len(marcas)})


def api_modelos_por_marca(request, marca_id):
    """API para obtener modelos por marca"""
    try:
        modelos = ModeloVehiculo.objects.filter(marca_id=marca_id, activo=True).values(
            "id", "nombre", "nombre_en", "tipo_vehiculo", "anio_inicio", "anio_fin"
        )

        # Agregar rango de años a cada modelo
        modelos_con_anos = []
        for modelo in modelos:
            modelo["anos_disponibles"] = list(
                range(modelo["anio_inicio"], (modelo["anio_fin"] or 2025) + 1)
            )
