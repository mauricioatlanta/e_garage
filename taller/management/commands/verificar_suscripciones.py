    ...
    def handle(self, *args, **options):
        ...
        for empresa in empresas_vencidas:
            try:
                if dry_run:
                    self.stdout.write(
                        f"[DRY-RUN] Desactivaría: {empresa.nombre_taller} (vencido: {empresa.fecha_fin})"
                    )
                else:
                    ...
                    html_message = render_to_string(
                        "email/suscripcion_vencida.html",
                        {
                            "empresa": empresa,
                            "plan": empresa.plan,
                ...
                self.stdout.write(
                    self.style.ERROR(f"❌ Error con {empresa.nombre_taller}: {str(e)}")
                )
                errores += 1
