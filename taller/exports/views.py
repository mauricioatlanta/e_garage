...
def exportar_documento_pdf(request, documento_id):
    ...
    try:
        ...
        messages.success(
            request,
            f"PDF generado exitosamente para {documento.tipo_documento} #{documento.numero_documento}",
        )
...
¡Gracias por confiar en nosotros! 🚗✨"""
        ...
        whatsapp_link = WhatsAppSender.generar_enlace_whatsapp(
            telefono=telefono, mensaje=mensaje, archivo_url=documento_url
        )
...
def exportar_rentabilidad_excel(request):
    ...
    try:
        ...
        messages.success(
            request,
            f"Reporte Excel generado para el período {fecha_inicio} - {fecha_fin}",
        )
        return response
...
def vista_previa_documento(request, documento_id):
    ...
    try:
        ...
        total_otros_servicios = sum(
            servicio.precio_cliente for servicio in documento.otros_servicios.all()
        )
