...
from taller.views.business_intelligence import (get_mecanicos_stats,
                                                get_repuestos_utilidad,
                                                get_servicios_ranking)
...
try:
    ...
    if servicios:
        for i, servicio in enumerate(servicios[:5], 1):
            print(
                f'  {i}. {servicio["nombre"]} - Vendidos: {servicio["cantidad_vendida"]} - Ingresos: ${servicio["ingresos_totales"]:,.0f}'
            )
    else:
    ...
    if repuestos:
        for i, repuesto in enumerate(repuestos[:5], 1):
            print(
                f'  {i}. {repuesto["nombre"]} - Utilidad: ${repuesto["utilidad_bruta"]:,.0f} ({repuesto["margen_utilidad"]}%)'
            )
    else:
    ...
    if mecanicos:
        for i, mecanico in enumerate(mecanicos, 1):
            print(
                f'  {i}. {mecanico["mecanico"].nombre} - Documentos: {mecanico["total_documentos"]} - Ingresos: ${mecanico["ingresos_totales"]:,.0f}'
            )
    else:
    ...
    total_repuestos = RepuestoDocumento.objects.filter(
        documento__empresa=empresa, documento__fecha__range=[fecha_inicio, fecha_fin]
    ).count()
    total_servicios = ServicioDocumento.objects.filter(
        empresa=empresa, documento__fecha__range=[fecha_inicio, fecha_fin]
    ).count()
    ...
    print(
        "  URL principal: http://localhost:8000/taller/business-intelligence/dashboard/"
    )
    print("  APIs disponibles:")
    print(
        "    - Servicios: http://localhost:8000/taller/business-intelligence/api/servicios-ranking/"
    )
    print(
        "    - Repuestos: http://localhost:8000/taller/business-intelligence/api/repuestos-utilidad/"
    )
    print(
        "    - Mecánicos: http://localhost:8000/taller/business-intelligence/api/mecanicos-stats/"
    )
    print()
