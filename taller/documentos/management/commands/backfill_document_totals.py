...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for doc in Documento.objects.all().prefetch_related('lineas_repuesto', 'lineas_servicio', 'lineas_otro_servicio'):
            count += 1
            ...
            needs_update = (
                doc.neto_repuestos != sum_rep or
                doc.neto_servicios != sum_serv or
                doc.neto_otros_servicios != sum_otros or
                doc.tax_amount != iva or
                doc.total != total
            )
        ...
        if dry_run:
            self.stdout.write(
                self.style.WARNING(f"✅ DRY-RUN completado: {count} documentos revisados, {updated} necesitarían actualización")
            )
        else:
            self.stdout.write(
                self.style.SUCCESS(f"✅ Recálculo completado: {updated} de {count} documentos actualizados")
            )
