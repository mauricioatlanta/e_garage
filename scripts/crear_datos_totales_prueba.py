...
try:
    ...
    cliente, created = Cliente.objects.get_or_create(
        rut="12345678-9",
        defaults={"nombre": "Cliente Prueba", "email": "prueba@test.com"},
    )
    if created:
    ...
    print(
        f"   - {linea_repuesto1.nombre}: {linea_repuesto1.cantidad} x ${linea_repuesto1.precio_unitario} = ${linea_repuesto1.subtotal}"
    )
    print(
        f"   - {linea_repuesto2.nombre}: {linea_repuesto2.cantidad} x ${linea_repuesto2.precio_unitario} (desc. {linea_repuesto2.descuento}%) = ${linea_repuesto2.subtotal}"
    )
    ...
    print(
        f"   - {linea_servicio1.nombre}: {linea_servicio1.cantidad} x ${linea_servicio1.precio_unitario} = ${linea_servicio1.subtotal}"
    )
    print(
        f"   - {linea_servicio2.nombre}: {linea_servicio2.cantidad} x ${linea_servicio2.precio_unitario} (desc. {linea_servicio2.descuento}%) = ${linea_servicio2.subtotal}"
    )
