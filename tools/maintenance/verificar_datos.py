...
if Tecnico.objects.exists():
    ...
    for mecanico in Tecnico.objects.all()[:5]:
        print(
            f"  - {mecanico.nombre} (Empresa: {mecanico.empresa.nombre_taller if mecanico.empresa else 'Sin empresa'})"
        )
...
if Documento.objects.exists():
    ...
    for doc in Documento.objects.all()[:5]:
        print(
            f"  - {doc.numero_documento} - {doc.tipo_documento} (Mecánico: {doc.mecanico.nombre if doc.mecanico else 'Sin mecánico'})"
        )
else:
