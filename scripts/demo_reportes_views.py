...
def demo_reportes_por_fecha(request):
    ...
    documentos = Documento.objects.filter(fecha_emision__range=[desde_date, hasta_date])
    ...
    top_servicios = (
        ...
        .annotate(total_veces=Count("id"), valor_total=Sum("precio_unitario"))
        .order_by("-total_veces")[:10]
    )

    # Top repuestos
    top_repuestos = (
        repuestos.values("nombre", "codigo")
        .annotate(
            cantidad_total=Sum("cantidad"),
            valor_total=Sum(F("cantidad") * F("precio_unitario")),
        )
