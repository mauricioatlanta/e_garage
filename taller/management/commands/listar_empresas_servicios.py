...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        empresas = Empresa.objects.annotate(total_servicios=Count("servicio")).order_by(
            "-total_servicios", "id"
        )
        ...
        for empresa in empresas:
            nombre = (
                empresa.nombre_taller[:28]
                if len(empresa.nombre_taller) > 28
                else empresa.nombre_taller
            )
            ...
            self.stdout.write(
                style(f"{empresa.id:<5} {nombre:<30} {servicios_count:<10} {empresa.pais:<5}")
            )
