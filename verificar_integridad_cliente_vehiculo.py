...
def verificar_integridad_datos():
    ...
    for pais in ["CL", "US"]:
        ...
        for vehiculo in vehiculos_pais[:3]:
            print(f"     - {vehiculo.patente} (Cliente: {vehiculo.cliente}, Empresa: {vehiculo.empresa})")
    ...
    if vehiculos_empresa_inconsistente.exists():
        print(f"   ❌ {vehiculos_empresa_inconsistente.count()} vehículos con empresa diferente a la del cliente")
    ...
    if cliente_usa:
        ...
        if vehiculos_cliente_total > vehiculos_cliente_empresa:
            print(f"   ⚠️ DIFERENCIA DETECTADA: {vehiculos_cliente_total - vehiculos_cliente_empresa} vehículos están en otra empresa")
            
            # Mostrar los vehículos problemáticos
            vehiculos_problema = Vehiculo.objects.filter(cliente=cliente_usa).exclude(empresa=cliente_usa.empresa)
            for v in vehiculos_problema:
                print(f"     - Vehículo {v.id} ({v.patente}): empresa={v.empresa}, cliente_empresa={cliente_usa.empresa}")
        else:
    ...
    total_inconsistencias = (
        vehiculos_empresa_inconsistente.count() + 
        len(documentos_problematicos)
    )
