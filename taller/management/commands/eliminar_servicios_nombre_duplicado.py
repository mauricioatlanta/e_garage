    ...
    def handle(self, *args, **options):
        ...
        for clave, servicios in duplicados.items():
            if len(servicios) > 1:
                ...
                self.stdout.write(self.style.WARNING(f"Eliminando duplicados de '{clave[0]}' en país {clave[1]} (manteniendo pk={a_mantener.pk})"))
                for s in a_eliminar:
                    ServicioName.objects.filter(servicio=s).delete()
                    self.stdout.write(f"  Eliminado servicio pk={s.pk}, subcategoria={s.subcategoria.get_label()}, categoria={s.subcategoria.categoria.get_label()} (code={s.code})")
                    s.delete()
                    eliminados += 1
        if eliminados == 0:
            self.stdout.write(self.style.SUCCESS("No había servicios con el mismo nombre en el mismo país para eliminar."))
        else:
            self.stdout.write(self.style.SUCCESS(f"Eliminados {eliminados} servicios duplicados por nombre y país."))
