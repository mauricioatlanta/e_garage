...
def test_timezone_and_i18n_filters_extra():
    try:
        from taller.templatetags import timezone_tags, simple_i18n
    except Exception:
    ...
    fmt_fn = getattr(timezone_tags, "format_datetime", None) or \
             getattr(timezone_tags, "tz_format", None) or \
             getattr(timezone_tags, "dt_format", None)
    if not fmt_fn:
