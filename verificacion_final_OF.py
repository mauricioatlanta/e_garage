...
from taller.models import Empresa, Cliente, Tecnico, Vehiculo, Repuesto, Documento, LineaRepuesto
...
def verificacion_final():
    ...
    print(f"✅ Repuesto OF: ID={repuesto_of.id}, nombre='{repuesto_of.nombre}', part_number='{repuesto_of.part_number}'")
    ...
    print(f"\n📄 ÚLTIMO DOCUMENTO:")
    print(f"DOC: {ultimo_doc.id} {ultimo_doc.empresa_id} {ultimo_doc.cliente_id} {ultimo_doc.vehiculo_id} {getattr(ultimo_doc, 'millas', None)}")
    ...
    # 4. Listar repuestos
    list_rep = list(ultimo_doc.lineas_repuesto.values_list("id","repuesto_id","nombre","cantidad","precio_unitario","descuento"))
...
def comandos_shell_manuales():
    ...
    print("print('Repuesto OF:', of_rep.id if of_rep else 'No existe', of_rep.nombre if of_rep else '')")
    print("")
    print("# Verificar último documento")
    print("doc = Documento.objects.latest('id')")
    print("print('DOC:', doc.id, doc.empresa_id, doc.cliente_id, doc.vehiculo_id, getattr(doc, 'millas', None))")
    ...
    print("print('LIST rep ids:', list(doc.lineas_repuesto.values_list('id','repuesto_id','nombre','cantidad','precio_unitario','descuento')))")
    print("")
