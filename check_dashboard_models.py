...
for doc in Documento.objects.select_related('empresa', 'cliente', 'tecnico').all()[:5]:
    print(f"- Doc #{doc.id}: {doc.tipo_documento} - Cliente: {doc.cliente} - Técnico: {doc.tecnico} - Empresa: {doc.empresa.nombre_taller}")

print()
print("⚙️ SERVICIOS MÁS POPULARES:")
from django.db.models import Count
servicios_top = LineaServicio.objects.values('nombre').annotate(
    total=Count('id')
).order_by('-total')[:5]
