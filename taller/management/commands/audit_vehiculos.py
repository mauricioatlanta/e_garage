    ...
    def handle(self, *args, **options):
        ...
        for v in vehiculos:
            ...
            if verbose:
                ...
                self.stdout.write(
                    f"   Cliente: {v.cliente} (ID: {v.cliente_id})"
                    if v.cliente
        ...
        if inconsistentes:
            ...
            empresa_inconsistente = [
                i for i in inconsistentes if i["tipo"] == "empresa_inconsistente"
            ]
    ...
    def corregir_inconsistencias(self, inconsistentes):
        ...
        for item in inconsistentes:
            ...
            if item["tipo"] == "sin_cliente":
                self.stdout.write(
                    f"   ⚠️ Vehículo {v.id} sin cliente - requiere intervención manual"
                )
                continue
                ...
                if v.cliente and v.cliente.empresa:
                    ...
                    v.save()
                    self.stdout.write(
                        f"   ✅ Vehículo {v.id}: {empresa_original} → {v.cliente.empresa}"
                    )
                    corregidos += 1
