    ...
    def handle(self, *args, **options):
        ...
        if company_name:
            try:
                ...
                raise CommandError(f'❌ No se encontró empresa con nombre que contenga: {company_name}')
        else:
        ...
        for empresa in empresas:
            ...
            documentos = Documento.objects.filter(
                empresa=empresa,
                estado_pago__in=['NO_PAGADO', 'PARCIAL']
            ...
            if dry_run:
                ...
                for doc in documentos[:5]:  # Mostrar solo los primeros 5 como ejemplo
                    self.stdout.write(f'      - {doc.tipo} #{doc.numero} - {doc.cliente.nombre} - ${doc.total}')
                if empresa_total > 5:
                    self.stdout.write(f'      ... y {empresa_total - 5} documentos más')
            else:
                # Realizar la actualización
                with transaction.atomic():
                    updated = documentos.update(
                        estado_pago='PAGADO',
                        pagado=True
                    )
        ...
        if dry_run:
            self.stdout.write(
                self.style.WARNING(f'🔍 MODO SIMULACIÓN: Se marcarían {total_documentos} documentos como pagados')
            )
            self.stdout.write('💡 Para aplicar los cambios, ejecuta sin --dry-run')
        else:
            self.stdout.write(
                self.style.SUCCESS(f'✅ PROCESO COMPLETADO: {documentos_actualizados} documentos marcados como pagados')
            )
