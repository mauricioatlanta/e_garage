...
class Command(BaseCommand):
    help = "Bootstrap demo para Venezuela (empresa, usuario, cliente, vehiculos, documento y desarme)"
    ...
    def handle(self, *args, **kwargs):
        ...
        empresa, empresa_created = Empresa.objects.get_or_create(user=user, defaults=empresa_defaults)
        ...
        if not vehiculo_created:
            for k, v in vehiculo_defaults.items():
        ...
        if not vd_created:
            for k, v in vehiculo_desarme_defaults.items():
                setattr(vehiculo_desarme, k, v)
            vehiculo_desarme.save()
