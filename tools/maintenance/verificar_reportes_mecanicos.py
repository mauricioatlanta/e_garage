    ...
    def handle(self, *args, **options):
        ...
        for empresa in empresas:
            ...
            for tecnico in tecnicos:
                ...
                total_generado = servicios_tecnico.aggregate(
                    total=Sum(
                ...
                if total_docs > 0:
                    ...
                    for doc in docs_tecnico[:3]:  # Mostrar solo los primeros 3
                        ...
                        total_doc = servicios_doc.aggregate(
                            total=Sum(
                                F('cantidad') * F('precio_unitario') * (1 - F('descuento') / 100)
                            )
                        )['total'] or 0
                        self.stdout.write(f'      - {doc.tipo} #{doc.numero}: ${total_doc:,.0f} ({servicios_doc.count()} servicios)')
