...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        with csv_path.open("r", encoding="utf-8-sig", newline="") as f:
            ...
            has_range = (
                "anio_desde" in normalized_fields
                or "anio_hasta" in normalized_fields
            )

            rows = list(reader)

        if has_single_year:
            self._import_from_year_rows(
                rows, dry_run=dry_run, clear=clear, chunk_size=chunk_size
            )
        elif has_range:
            self._import_from_range_rows(
                rows, dry_run=dry_run, clear=clear, chunk_size=chunk_size
            )
        else:
            raise CommandError(
    ...
    def _import_from_year_rows(
        self, rows, dry_run=False, clear=False, chunk_size=5000
    ):
        grouped = defaultdict(set)
        skipped = 0

        for idx, row in enumerate(rows, start=2):
            marca = self._normalize_brand(
                self._get_value(row, "marca", "Marca", "make", "Make")
            )
            modelo = self._normalize_model(
                self._get_value(row, "modelo", "Modelo", "model", "Model")
            )
            anio_raw = self._get_value(
                row, "anio", "Anio", "Año", "year", "Year"
            )
    ...
    def _import_from_range_rows(
        ...
        for idx, row in enumerate(rows, start=2):
            ...
            if (
                anio_desde is not None
                and anio_hasta is not None
    ...
    def _save_objects(
        ...
        if dry_run:
            ...
            if len(objects) > 20:
                self.stdout.write(
                    self.style.WARNING(f"  ... y {len(objects) - 20} más")
                )
            return
        ...
        with transaction.atomic():
            if clear:
                ...
                self.stdout.write(
                    self.style.WARNING(
                        f"Catálogo eliminado: {deleted} registros"
                    )
                )
