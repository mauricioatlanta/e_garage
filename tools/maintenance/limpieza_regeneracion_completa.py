...
def crear_marcas_modelos():
    ...
    marca_chile, created = MarcaVehiculo.objects.get_or_create(
        nombre="Chevrolet", defaults={"pais": "CL", "activo": True}
    )
    if created:
    ...
    marca_usa, created = MarcaVehiculo.objects.get_or_create(
        nombre="Ford", defaults={"pais": "US", "activo": True}
    )
    if created:
        ...
        with transaction.atomic():
            for usuario_data in usuarios_creados:
                    ...
                    clientes_data = [
                        {
                            "nombre": "John Smith",
                            "rut": "SSN123456789",
                            "telefono": "+15551111111",
                        },
                        {
                            "nombre": "Jane Doe",
                            "rut": "SSN987654321",
                            "telefono": "+15552222222",
                        },
                    ]
                ...
                for i, cliente in enumerate(clientes):
                    ...
                    print_success(
                        f"  Vehículo creado: {vehiculo.marca.nombre} {vehiculo.modelo.nombre} {vehiculo.anio}"
                    )
                ...
                for i, tipo_doc in enumerate(tipos_doc):
                    ...
                    print_success(
                        f"  Documento creado: {documento.numero_documento} - Total: ${total:,.0f}"
                    )
