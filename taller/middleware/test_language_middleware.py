    ...
    def process_request(self, request):
        ...
        if path.startswith("/us/"):
            ...
            if "/es/" in path:
                ...
                print(
                    f"🇺🇸 TestLanguageMiddleware: Setting default language to 'en' for path: {path}"
                )
                logger.info(
                    f"🇺🇸 TestLanguageMiddleware: Setting default language to 'en' for path: {path}"
                )
    ...
    def process_response(self, request, response):
        if request.path and request.path.startswith("/us/"):
            ...
            if "/es/" in request.path:
                ...
                logger.info(
                    f"🇪🇸 TestLanguageMiddleware: Set Content-Language to 'es' for {request.path}"
                )
            else:
                response.headers["Content-Language"] = "en"
                logger.info(
                    f"🇺🇸 TestLanguageMiddleware: Set Content-Language to 'en' for {request.path}"
                )
        return response
