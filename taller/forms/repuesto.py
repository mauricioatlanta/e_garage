...
from taller.models.repuesto import Repuesto, CategoriaRepuesto
...
class RepuestoForm(forms.ModelForm):
    def __init__(self, *args, **kwargs):
        ...
        if is_english:
            ...
            english_categories = [
                'Engine', 'Brake', 'Electrical', 'Suspension', 'Transmission',
                'Exhaust', 'Cooling', 'Fuel', 'Body', 'Tires', 'Filters', 'Oils'
            ]
            self.fields['categoria'].queryset = CategoriaRepuesto.objects.filter(
                empresa=self.user.empresa,
                nombre__in=english_categories
            )
        else:
            ...
            spanish_categories = [
                'Motor', 'Frenos', 'Sistema Eléctrico', 'Suspensión', 'Transmisión',
                'Sistema de Escape', 'Refrigeración', 'Combustible', 'Carrocería', 
                'Neumáticos', 'Filtros', 'Aceites y Lubricantes'
            ]
            self.fields['categoria'].queryset = CategoriaRepuesto.objects.filter(
                empresa=self.user.empresa,
                nombre__in=spanish_categories
            )
    ...
    class Meta:
        ...
        widgets = {
            'part_number': forms.TextInput(attrs={'class': 'w-full bg-black/40 text-white border border-cyan-400/30 rounded-lg p-2'}),
            'nombre': forms.TextInput(attrs={'class': 'w-full bg-black/40 text-white border border-cyan-400/30 rounded-lg p-2'}),
            'categoria': forms.Select(attrs={'class': 'w-full bg-black/40 text-white border border-cyan-400/30 rounded-lg p-2'}),
            'precio_compra': forms.TextInput(attrs={'class': 'w-full bg-black/40 text-white border border-cyan-400/30 rounded-lg p-2', 'inputmode': 'decimal'}),
            'precio_venta': forms.TextInput(attrs={'class': 'w-full bg-black/40 text-white border border-cyan-400/30 rounded-lg p-2', 'inputmode': 'decimal'}),
            'cantidad_stock': forms.NumberInput(attrs={'class': 'w-full bg-black/40 text-white border border-cyan-400/30 rounded-lg p-2'}),
            'proveedor': forms.TextInput(attrs={'class': 'w-full bg-black/40 text-white border border-cyan-400/30 rounded-lg p-2'}),
        }
    ...
    def clean_precio_compra(self):
        ...
        if separador_decimal and '.' in limpio:
            ...
            try:
                ...
                is_english = self.user and hasattr(self.user, 'empresa') and self.user.empresa.pais == 'US'
                error_msg = "Purchase price must be a valid number" if is_english else "El precio de compra debe ser un número válido"
    ...
    def clean_precio_venta(self):
        ...
        if separador_decimal and '.' in limpio:
            ...
            try:
                ...
                is_english = self.user and hasattr(self.user, 'empresa') and self.user.empresa.pais == 'US'
                error_msg = "Sale price must be a valid number" if is_english else "El precio de venta debe ser un número válido"
                raise forms.ValidationError(error_msg)
        elif separador_decimal and ',' in limpio and '.' not in limpio:
            # Para formato europeo (comas como separador decimal)
            limpio = limpio.replace(',', '.')  # Convertir coma a punto
    ...
    def clean_part_number(self):
        ...
        if self.user and hasattr(self.user, 'empresa'):
            ...
            if existing_repuestos.exists():
                ...
                error_msg = f"Part number '{part_number}' already exists for this company" if is_english else f"El número de parte '{part_number}' ya existe para esta empresa"
                raise forms.ValidationError(error_msg)
