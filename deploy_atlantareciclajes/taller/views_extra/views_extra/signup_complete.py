...
def signup_complete(request):
    ...
    # 🎯 DETECTAR SI VIENE DESDE "PROBAR GRATIS" (parámetro trial=1)
    is_trial_mode = request.GET.get("trial") == "1"
    ...
    if from_country == "cl":
        ...
        language_code = "es"
        request.session["django_language"] = "es"
    elif from_country == "co":
        activate("es")  # Español para Colombia
        initial_country = "CO"
        language_code = "es"
        request.session["django_language"] = "es"
    elif from_country == "ec":
        activate("es")  # Español para Ecuador
        initial_country = "EC"
        language_code = "es"
        request.session["django_language"] = "es"
    ...
    if request.method == "POST":
        ...
        # 🎯 PRE-SELECCIONAR PAÍS Y PLAN SEGÚN URL
        initial_data = {"pais": initial_country}
        # Si viene desde "Probar Gratis", preseleccionar plan trial
        if is_trial_mode:
            initial_data["plan"] = "trial"
        form = SignupCompleteForm(initial=initial_data)

    # Precios para mostrar en el template
    context = {
        "form": form,
        "from_country": from_country,  # 'cl', 'us', 'mx', 'pe', etc.
        "language_code": language_code,  # 'es' o 'en'
        "is_trial_mode": is_trial_mode,  # True si viene desde botón "Probar Gratis"
        "precios": {
            "CL": {
    ...
    # Seleccionar template según país e idioma (igual que el login)
    country = initial_country
    lang = language_code

    if country == "US":
        if lang == "es":
            template_name = "cl/es/account/signup.html"
        else:
            template_name = "us/en/account/signup.html"
    elif country == "MX":
        if lang == "en":
            template_name = "us/en/account/signup.html"
        else:
            template_name = "cl/es/account/signup.html"  # Por ahora usar el de CL, luego crear uno específico para MX
