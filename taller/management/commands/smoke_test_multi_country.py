...
def reverse_country_lang(country: str, lang: str, view_name: str, *args, verbose=False, **kwargs):
    ...
    variants = [
        ...
        (f"{country_ns}:taller:{view_name}", "fallback_taller"),  # chile:taller:clientes:lista_clientes
    ...
    if verbose and resolved_variant:
        ...
        logger.debug(f"[reverse_country_lang] {country}/{lang} - {view_name} → {resolved_variant} → {resolved_url}")
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if country_filter:
            countries_to_test = [
                (c, l) for c, l in countries_to_test 
                if c.upper() == country_filter.upper()
            ]
        ...
        try:
            ...
            empresa_a = empresas_by_country.get('CL', empresas_by_country[list(empresas_by_country.keys())[0]])
            ...
            self.log_success(f'Empresas y usuarios de prueba creados ({len(empresas_by_country)} países)')
        ...
        try:
            cliente_a, vehiculo_a, repuesto_a, documento_a = self._create_test_data(empresa_a, user_a)
        ...
        self._test_multi_tenant_isolation(user_a, user_b, cliente_a, vehiculo_a, repuesto_a, documento_a, verbose)
        ...
        if not skip_dynamic_lists:
            ...
            self._test_dynamic_lists(user_a, empresa_a, empresas_by_country, users_by_country, verbose)
    ...
    def _test_multi_tenant_isolation(self, user_a, user_b, cliente_a, vehiculo_a, repuesto_a, documento_a, verbose):
        ...
        for model_name, view_name, kwargs, ModelClass in tests:
            try:
                ...
                if response_get.status_code == 200:
                    ...
                    has_method_post = 'method="post"' in content or "method='post'" in content or 'method=post' in content
                    # Si no tiene method explícito, HTML default es GET, así que debe tener method="post"
                    form_is_post = has_method_post or (has_form and 'method="get"' not in content and "method='get'" not in content)
                    
                    # Buscar CSRF token (puede estar en input o como variable)
                    has_csrf = 'csrfmiddlewaretoken' in content or 'csrf_token' in content or 'csrf' in content
                    
                    # Buscar botón/enviar (submit)
                    has_submit = 'type="submit"' in content or "type='submit'" in content or '<button' in content or 'input type="submit"' in content
                    
                    if has_form and form_is_post and has_csrf and has_submit:
                        self.log_success(f'{model_name}: GET muestra confirmación con form POST + CSRF + submit')
                    else:
                        missing = []
                        if not has_form:
                            missing.append('form')
                ...
                if csrf_token:
                    ...
                    if response_post.status_code in [200, 302, 204]:
                        ...
                        if not obj_exists:
                            self.log_success(f'{model_name}: POST eliminó correctamente (objeto no existe)')
                        else:
                    ...
                    self.log_warning(f'{model_name}: No se pudo obtener CSRF token para probar POST')
    ...
    def _test_crud_by_country(self, user_a, empresa_a, country, lang, verbose):
        ...
        if empresa_a.pais != country:
            self.log_warning(f'{country}/{lang} - Empresa tiene país {empresa_a.pais}, esperado {country}')
        ...
        for model_name, view_name in list_tests:
            try:
                ...
                if response.status_code == 200:
                    ...
                    self.log_error(f'{country}/{lang} - {model_name}: Listar falló (status {response.status_code})')
                if verbose:
                    self.stdout.write(f'  GET {url} → {response.status_code}')
            except NoReverseMatch:
                self.log_warning(f'{country}/{lang} - {model_name}: URL no encontrada (reverse falló)')
            except Exception as e:
        ...
        for model_name, view_name in create_tests:
            try:
                ...
                response = client.get(url)
                if response.status_code == 200:
                    self.log_success(f'{country}/{lang} - {model_name}: Formulario crear carga (200)')
                else:
                    self.log_warning(f'{country}/{lang} - {model_name}: Formulario crear retornó {response.status_code}')
                if verbose:
                    self.stdout.write(f'  GET {url} → {response.status_code}')
            except NoReverseMatch:
    ...
    def _test_dynamic_lists(self, user_a, empresa_a, empresas_by_country, users_by_country, verbose):
        """
        ...
        for country, lang in country_lang_tests:
            ...
            try:
                url = reverse_country_lang(country, lang, "clientes:obtener_ciudades", verbose=verbose)
                if verbose:
                ...
                if response.status_code == 200:
                    try:
                        ...
                        if isinstance(data, list):
                            if len(data) > 0:
                                self.log_success(f'{country}/{lang} - Ciudades por {"región" if country == "CL" else "estado"}: OK (payload no vacío)')
                            else:
                                # 🔒 WARNING: Payload vacío puede ser falta de seeds
                                self.log_warning(f'{country}/{lang} - Ciudades: payload vacío (posible falta de seeds en DB)')
                        else:
                            self.log_warning(f'{country}/{lang} - Ciudades: respuesta no es lista JSON')
                    ...
                    self.log_error(f'{country}/{lang} - Ciudades: status {response.status_code} (esperado 200)')
            except Exception as e:
                self.log_warning(f'{country}/{lang} - Ciudades: error - {e}')
            
            # Endpoint de modelos por marca (todos los países)
            try:
                url = reverse_country_lang(country, lang, "vehiculos:ajax_modelos_por_marca_anio", verbose=verbose)
                if verbose:
                ...
                if response.status_code == 200:
                        ...
                        if isinstance(data, (list, dict)):
                            ...
                            if isinstance(data, list) and len(data) > 0:
                                ...
                                if 'country' in first_item and first_item['country'] != country:
                                    self.log_error(f'{country}/{lang} - Modelos: devuelve datos de otro país ({first_item["country"]} != {country})')
                                else:
                                    self.log_success(f'{country}/{lang} - Modelos por marca: OK (payload no vacío)')
                            elif isinstance(data, dict) and 'results' in data and len(data.get('results', [])) > 0:
                                self.log_success(f'{country}/{lang} - Modelos por marca: OK (payload no vacío)')
                            else:
                                # 🔒 WARNING: Payload vacío puede ser falta de seeds
                                self.log_warning(f'{country}/{lang} - Modelos: payload vacío (posible falta de seeds en DB)')
                        else:
                            self.log_warning(f'{country}/{lang} - Modelos: respuesta no es JSON válido (list/dict)')
                    except json.JSONDecodeError:
                    ...
                    self.log_error(f'{country}/{lang} - Modelos: status {response.status_code} (esperado 200)')
            except Exception as e:
    ...
    def _print_summary(self):
        ...
        self.stdout.write(f'🌍 Países probados: {len(self.tested_countries)} ({", ".join(self.tested_countries)})')
