...
def test_formularios_inteligentes():
    ...
    try:
        ...
        print(
            f"✅ Usuario Chile: {user_chile.username} - Empresa: {user_chile.empresa.nombre}"
        )
        print(
            f"✅ Usuario USA: {user_usa.username} - Empresa: {user_usa.empresa.nombre}"
        )
    ...
    if hasattr(form_vehiculo_cl.fields["marca"], "queryset"):
        marcas_cl = list(
            form_vehiculo_cl.fields["marca"].queryset.values_list("nombre", flat=True)
        )[:5]
        print(f"   • Marcas disponibles (primeras 5): {marcas_cl}")

    patente_placeholder_cl = form_vehiculo_cl.fields["patente"].widget.attrs.get(
        "placeholder", "N/A"
    )
    ...
    if hasattr(form_vehiculo_us.fields["marca"], "queryset"):
        marcas_us = list(
            form_vehiculo_us.fields["marca"].queryset.values_list("nombre", flat=True)
        )[:5]
        print(f"   • Marcas disponibles (primeras 5): {marcas_us}")

    patente_placeholder_us = form_vehiculo_us.fields["patente"].widget.attrs.get(
        "placeholder", "N/A"
    )
    ...
    telefono_placeholder_cl = form_cliente_cl.fields["telefono"].widget.attrs.get(
        "placeholder", "N/A"
    )
    ...
    telefono_placeholder_us = form_cliente_us.fields["telefono"].widget.attrs.get(
        "placeholder", "N/A"
    )
    ...
    precio_compra_placeholder_cl = form_repuesto_cl.fields[
        "precio_compra"
    ].widget.attrs.get("placeholder", "N/A")
    precio_venta_placeholder_cl = form_repuesto_cl.fields[
        "precio_venta"
    ].widget.attrs.get("placeholder", "N/A")
    ...
    precio_compra_placeholder_us = form_repuesto_us.fields[
        "precio_compra"
    ].widget.attrs.get("placeholder", "N/A")
    precio_venta_placeholder_us = form_repuesto_us.fields[
        "precio_venta"
    ].widget.attrs.get("placeholder", "N/A")
    ...
    if form_vehiculo_test_cl.is_valid():
        ...
        print(
            f"   ❌ Error patente: {form_vehiculo_test_cl.errors.get('patente', 'N/A')}"
        )
    ...
    if form_cliente_test_us.is_valid():
        ...
        print(
            f"   ❌ Error teléfono: {form_cliente_test_us.errors.get('telefono', 'N/A')}"
        )
