    ...
    def handle(self, *args, **options):
        try:
            ...
            try:
                ...
                self.stdout.write(
                    self.style.SUCCESS(f'✅ Empresa creada: {empresa.nombre_taller} (País: {empresa.pais})')
                )
            ...
            if empresa.pais == 'US':
                self.stdout.write(
                    self.style.SUCCESS('🇺🇸 ¡Corrección completada! El usuario ahora debería ver la bandera de USA.')
                )
            else:
                self.stdout.write(
