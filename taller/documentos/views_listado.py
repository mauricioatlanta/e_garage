...
class DocumentoListViewBase(ListView):
    ...
    def get_queryset(self):
        ...
        rep_ct = Subquery(
            ...
            .values("documento").annotate(c=Count("id"))
            .values("c")[:1],
        ...
        rep_sum = Subquery(
            LineaRepuesto.objects.filter(documento_id=OuterRef("pk"))
            .values("documento").annotate(s=Sum("precio_unitario"))
            .values("s")[:1],
        ...
        otros_sum = Subquery(
            LineaOtroServicio.objects.filter(documento_id=OuterRef("pk"))
            .values("documento").annotate(s=Sum("precio_cliente"))
            .values("s")[:1],
            output_field=DecimalField(max_digits=12, decimal_places=2),
        )
        ...
        return (
            qs.select_related("empresa", "cliente", "vehiculo", "tecnico_responsable")
              .annotate(
                  ...
                  sum_rep=Coalesce(rep_sum, Value(0), output_field=DecimalField(max_digits=12, decimal_places=2)),
                  sum_serv=Coalesce(serv_sum, Value(0), output_field=DecimalField(max_digits=12, decimal_places=2)),
                  sum_out=Coalesce(otros_sum, Value(0), output_field=DecimalField(max_digits=12, decimal_places=2)),
              )
