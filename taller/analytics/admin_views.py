...
def dashboard_admin(request):
    ...
    por_vencer_3_dias = Empresa.objects.filter(
        fecha_fin__lte=proximos_3_dias, fecha_fin__gte=hoy, suscripcion_activa=True
    ).count()
    ...
    trials_pendientes = TrialRegistro.objects.filter(
        prueba_activa=False, prueba_expirada=False
    ).count()
    ...
    ingresos_mes_actual = (
        Empresa.objects.filter(
            suscripcion_activa=True, plan__in=["basic", "premium", "enterprise"]
    ...
    for i in range(8):
        ...
        nuevos_semana = Empresa.objects.filter(
            fecha_inicio__gte=inicio_semana, fecha_inicio__lt=fin_semana
        ).count()
...
def detalle_suscriptor(request, empresa_id):
    ...
    try:
        ...
        info_adicional = {
            ...
            "dias_desde_registro": (
                (ahora.date() - empresa.user.date_joined.date()).days
                if empresa.user.date_joined
                else 0
            ),
        }
...
def dashboard_avanzado(request):
    ...
    return render(
        request,
        "analytics/dashboard_avanzado.html",
        {
