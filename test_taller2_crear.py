...
def test_crear_documento_taller2():
    ...
    json_items = json.dumps(
        [
            ...
            {"tipo": "servicio", "nombre": "Servicio TEST-TALLER2", "precio": 35000},
        ]
    )
    ...
    if response.status_code == 302:  # Redirect después de crear
        ...
        documento_creado = (
            Documento.objects.filter(empresa=empresa_taller2).order_by("-id").first()
        )

        if documento_creado:
            print(
                f"📋 Documento creado: #{documento_creado.id} - {documento_creado.numero_documento}"
            )
            ...
            if repuestos.exists() or servicios.exists():
                ...
                print(
                    f"🔗 URL: http://127.0.0.1:8000/documentos/{documento_creado.id}/"
                )
            else:
