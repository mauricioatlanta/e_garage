    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if Empresa.objects.filter(user=user).exists():
                messages.warning(
                    request, "Ya tienes una empresa registrada asociada a tu usuario."
                )
                return redirect(
                    "nombre_vista_deseada"
                )  # Cambia por la vista que corresponda

            # Crear empresa automáticamente para el usuario
            empresa = Empresa.objects.create(
                user=user, nombre_taller=nombre_taller, email=email
            )
            ...
            TallerInfo.objects.create(
                ...
                telefono=telefono,
                ha_usado_prueba=ha_usado_prueba,
            )
