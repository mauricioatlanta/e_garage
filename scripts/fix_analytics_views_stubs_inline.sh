...
# Ejecutar directamente en el servidor: sudo bash fix_analytics_views_stubs_inline.sh
...
if grep -q "# --- HOTFIX: stubs para compatibilidad con urls.py ---" "$VIEWS_FILE"; then
    echo "⚠️  Los stubs ya están presentes. Eliminando stubs anteriores..."
    # Eliminar stubs anteriores si existen (desde la línea del comentario hasta el final)
