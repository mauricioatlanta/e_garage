...
def test_herencia_responsable_on_documento():
    ...
    if response.status_code in (200, 201):
        ...
        for linea in lineas_servicio:
            assert linea.tecnico_responsable == tecnico, f"Service line should inherit tecnico_responsable: {linea.nombre}"
        ...
        if lineas_repuesto.exists():
            for linea in lineas_repuesto:
                assert linea.tecnico_responsable == tecnico, f"Repuesto line should inherit tecnico_responsable: {linea.nombre}"
...
def test_herencia_responsable_on_vs_off():
    ...
    if response_off.status_code in (200, 201):
        ...
        assert linea_off.tecnico_responsable == tecnico_linea, "OFF: Line should use its own tecnico"
...
def test_herencia_responsable_edge_cases():
    ...
    if response.status_code in (200, 201, 302):
        ...
        assert response.status_code in (400, 422), f"Should handle no tecnico gracefully: {response.status_code}"
    ...
    if response.status_code in (200, 201):
        ...
        assert linea.tecnico_responsable is None, "Line should have None tecnico when document has None"
