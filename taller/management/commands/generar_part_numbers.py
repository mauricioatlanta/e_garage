...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        ) | Repuesto.objects.filter(part_number="")
        total = repuestos.count()

        if total == 0:
            self.stdout.write(
                self.style.SUCCESS("✅ Todos los repuestos ya tienen un part_number.")
            )
            return
        ...
        self.stdout.write(
            self.style.SUCCESS(f"✅ Se asignaron part_numbers a {total} repuestos.")
        )
