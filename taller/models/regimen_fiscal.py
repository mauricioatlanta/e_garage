...
class RegimenFiscal(models.Model):
    ...
    nombre_impuesto = models.CharField(
        max_length=50, help_text=_("Nombre del impuesto (IVA, ISS, Sales Tax, IGV, etc.)")
    )

    nombre_impuesto_en = models.CharField(
        max_length=50, blank=True, help_text=_("Nombre del impuesto en inglés (opcional)")
    )
    ...
    descripcion = models.TextField(
        blank=True, help_text=_("Descripción adicional del régimen fiscal")
    )
...
def crear_regimenes_fiscales_default():
    ...
    for regimen_data in regimenes:
        regimen, created = RegimenFiscal.objects.get_or_create(
            pais=regimen_data["pais"], defaults=regimen_data
        )
        if created:
