...
def track_company_settings_changes(sender, instance, **kwargs):
    ...
    if instance.pk:  # Solo para updates, no para creates
        try:
            ...
            # Campos a trackear
            tracked_fields = [
                'company_name', 'tagline', 'logo', 'primary_color', 'secondary_color',
                'address', 'phone', 'email', 'website', 'tax_id', 'business_license',
                'currency', 'invoice_prefix', 'quote_prefix', 'work_order_prefix',
                'about_text', 'terms_and_conditions'
            ]
