...
print(f"  swaks --to test@dominio.com --server {HOST} --port 587 --auth LOGIN --auth-user suscripcion@atlantareciclajes.cl --tls\n")
...
try:
    ...
    if HOST not in mx_hosts:
        print(f"  ⚠️  ATENCIÓN: {HOST} no es un MX directo del dominio. Puede haber relay o restricción.")
    ...
    for user in USERS:
        for auth_method in AUTH_METHODS:
            ...
            try:
                ...
                results.append((port, user, auth_method, False, f"AUTH {e.smtp_code} {e.smtp_error}"))
...
print("- Si todos los métodos fallan, revisa si hay bloqueo por IP, límite de conexiones o si el buzón tiene habilitado SMTP.")
print("- Si tienes acceso al panel de control, revisa logs de acceso y errores SMTP para más detalles.")
