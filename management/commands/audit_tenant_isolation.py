...
class Command(BaseCommand):
    ...
    def audit_clientes(self, Cliente):
        ...
        if duplicated_emails.exists():
            self.stdout.write(
                self.style.WARNING(f'⚠️ {duplicated_emails.count()} emails compartidos entre empresas')
            )
            if self.verbose:
                for item in duplicated_emails[:5]:
                    ...
                    for c in clientes:
                        self.stdout.write(f'    * Empresa: {c.empresa.nombre if c.empresa else "SIN EMPRESA"}')
    ...
    def audit_vehiculos(self, Vehiculo):
                ...
                with transaction.atomic():
                    for vehiculo in bad_vehiculos:
                        if vehiculo.cliente and vehiculo.cliente.empresa:
                            ...
                            fixed += 1
                            if self.verbose:
                                self.stdout.write(f'  ✅ Corregido vehículo {vehiculo.pk} -> empresa {vehiculo.empresa.nombre}')
        ...
        if count > 0:
            ...
            if self.fix_mode:
                ...
                self.stdout.write(self.style.SUCCESS(f'✅ Corregidos {fixed} vehículos inconsistentes'))
        else:
            self.stdout.write(self.style.SUCCESS('✅ Todos los vehículos tienen empresa consistente'))
    ...
    def audit_documentos(self, Documento):
        ...
        if count > 0:
            self.stdout.write(
                self.style.ERROR(f'❌ {count} documentos sin empresa')
            )
        ...
        if count > 0:
            ...
            if self.fix_mode:
                ...
                self.stdout.write(self.style.SUCCESS(f'✅ Corregidos {fixed} documentos inconsistentes'))
    ...
    def audit_repuestos(self, Repuesto):
        ...
        if hasattr(Repuesto, 'empresa'):
            ...
            if count > 0:
                self.stdout.write(
                    self.style.ERROR(f'❌ {count} repuestos sin empresa')
                )
            else:
    ...
    def audit_servicios(self, Servicio):
        ...
        if hasattr(Servicio, 'empresa'):
            ...
            if count > 0:
                self.stdout.write(
                    self.style.ERROR(f'❌ {count} servicios sin empresa')
                )
            else:
    ...
    def audit_lineas_documento(self):
        ...
        try:
            from taller.documentos.models import LineaRepuesto, LineaServicio, LineaOtroServicio
            ...
            if hasattr(LineaRepuesto, 'empresa'):
                ...
                if count > 0:
                    ...
                    if self.fix_mode:
                        ...
                        self.stdout.write(self.style.SUCCESS(f'✅ Corregidas {fixed} líneas de repuesto'))
                else:
            ...
            if hasattr(LineaServicio, 'empresa'):
                ...
                if count > 0:
                    ...
                    if self.fix_mode:
                        ...
                        self.stdout.write(self.style.SUCCESS(f'✅ Corregidas {fixed} líneas de servicio'))
                else:
