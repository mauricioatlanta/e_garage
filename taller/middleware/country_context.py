...
class CountryContextMiddleware(MiddlewareMixin):
    ...
    def process_request(self, request):
        country = self._detect_country_from_url(request)
        
        if not country:
            country = self._detect_country_from_subdomain(request)
            
        if not country:
            country = self._detect_country_from_user(request)
...
class LanguageContextMiddleware(MiddlewareMixin):
    ...
    def process_request(self, request):
        ...
        # Debug temporal
        if getattr(settings, 'DEBUG', False):
            print(f"🗣️ LanguageContext: {language} (País: {getattr(request, 'country', 'N/A')})")
