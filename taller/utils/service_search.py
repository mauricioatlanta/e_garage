...
class ServiceSearchEngine:
    ...
    def search_services(self, query, limit=10):
        ...
        results.extend([(service, 80) for service in prefix_matches if service not in [r[0] for r in results]])
        
        # 3. Búsqueda en aliases
        alias_matches = self._search_aliases(normalized_query)
        results.extend([(service, 70) for service in alias_matches if service not in [r[0] for r in results]])
        
        # 4. Búsqueda fuzzy
        fuzzy_matches = self._search_fuzzy(normalized_query)
        results.extend([(service, score) for service, score in fuzzy_matches if service not in [r[0] for r in results]])
    ...
    def _search_fuzzy(self, query, min_similarity=0.6):
        ...
        for service_name in service_names:
            ...
            if similarity >= min_similarity:
                services_with_scores.append((service_name.servicio, int(similarity * 60)))  # Max score 60 for fuzzy
            ...
            for alias in aliases:
                ...
                if similarity >= min_similarity:
                    services_with_scores.append((service_name.servicio, int(similarity * 55)))  # Max score 55 for fuzzy alias
                    break
