...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not regiones.exists():
            self.stdout.write(self.style.ERROR('❌ No hay regiones en la base de datos. Ejecuta primero: python manage.py cargar_regiones'))
            return
        ...
        if total_clientes == 0:
            self.stdout.write(self.style.SUCCESS('✅ Todos los clientes ya tienen región y ciudad asignadas'))
            return
        ...
        for cliente in clientes_qs:
            try:
                ...
                if not ciudades_region.exists():
                    self.stdout.write(self.style.WARNING(f'⚠️  La región {region.nombre} no tiene ciudades. Creando ciudad por defecto...'))
                    ciudad = TallerCiudad.objects.create(
                        nombre=f"{region.nombre} - Ciudad Principal",
                        region=region
                    )
                else:
                ...
                if actualizados % 10 == 0:
                    self.stdout.write(f'  ✓ Actualizados {actualizados}/{total_clientes} clientes...')
                    
            except Exception as e:
                errores += 1
                self.stdout.write(self.style.ERROR(f'  ✗ Error al actualizar cliente {cliente.pk}: {str(e)}'))
