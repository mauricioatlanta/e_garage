...
def assert_ok_or_redirect(resp, expected_redirect_suffix=None):
    ...
    if expected_redirect_suffix:
        ...
        assert location.endswith(expected_redirect_suffix), f"Redirect to {location} doesn't end with {expected_redirect_suffix}"
...
def assert_json_response(resp, expected_status_codes=(200, 201)):
    ...
    """
    assert resp.status_code in expected_status_codes, f"Expected {expected_status_codes}, got {resp.status_code}: {resp.content}"
...
def assert_redirect_preserves_querystring(resp, expected_base_path, expected_params=None):
    ...
    location = resp.headers.get("Location", "")
    assert location.startswith(expected_base_path), f"Redirect to {location} doesn't start with {expected_base_path}"
    ...
    if expected_params:
        ...
        for key, expected_value in expected_params.items():
            ...
            assert actual_value == expected_value, f"Query param {key}: expected {expected_value}, got {actual_value}"
