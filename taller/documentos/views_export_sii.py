...
def exportar_csv_sii(request, documento_id):
    ...
    if not documento.cliente:
        return JsonResponse(
            {"success": False, "error": "El documento no tiene cliente asignado"}, status=400
        )
    ...
    subtotal_repuestos = sum(
        linea.cantidad * linea.precio_unitario for linea in documento.lineas_repuesto.all()
    )
    subtotal_servicios = sum(linea.precio_unitario for linea in documento.lineas_servicio.all())
    subtotal_otros = sum(linea.precio_cliente for linea in documento.lineas_otro_servicio.all())
