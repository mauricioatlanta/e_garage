...
class TipoNotificacion(models.Model):
    ...
    dias_anticipacion = models.PositiveIntegerField(
        default=0, help_text="Días de anticipación para el recordatorio"
    )
...
class ConfiguracionNotificacion(models.Model):
    ...
    empresa = models.OneToOneField(
        Empresa, on_delete=models.CASCADE, related_name="config_notificaciones"
    )
    ...
    email_password = models.CharField(
        max_length=100, blank=True, help_text="Password de aplicación"
    )
    ...
    sms_proveedor = models.CharField(
        max_length=50,
        default="twilio",
        choices=[
            ("twilio", "Twilio"),
...
class RecordatorioMantenimiento(models.Model):
    ...
    def requiere_notificacion(self):
        ...
        return (
            not self.notificacion_enviada
            and self.estado == "PROGRAMADO"
            and dias_restantes is not None
            and dias_restantes <= self.dias_recordatorio
        )
