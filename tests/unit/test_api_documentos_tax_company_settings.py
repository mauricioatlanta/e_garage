...
def test_company_settings_tasa_sobrescribe_fallback():
    ...
    url = _rev(["taller:documentos_api_create","documentos:api_create"],
    ...
    def _mk_emp(pais, tasa_setting):
        ...
        ok = _set_any(cs, {"iva": tasa_setting, "iva_porcentaje": tasa_setting,
                           "sales_tax": tasa_setting, "tax_rate": tasa_setting, "tasa_iva": tasa_setting})
        if not ok:
        ...
        veh = Vehiculo.objects.create(empresa=emp, cliente=cli, patente=f"P{pais}123",
                                      marca_texto="X", modelo_texto="Y", anio=2021)
        ...
        payload = {
            ...
            "tipo": "FAC", "fecha_emision": "2025-01-10",
            "lineas_servicio": [{"nombre":"Srv","cantidad":1,"precio_unitario":15000,"descuento":0}],
            "lineas_repuesto": [{"nombre":"Rep","cantidad":2,"precio_unitario":5000,"descuento":0}],
        }
