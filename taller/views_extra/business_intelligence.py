...
from taller.models.lineas_documento import LineaRepuesto as RepuestoDocumento, LineaServicio as LineaServicio
    ...
    try:
        ...
        for tecnico in tecnicos:
            ...
            try:
                ...
                total_documentos = documentos.count() // max(tecnicos.count(), 1)  # Distribuir equitativamente
...
def get_resumen_general(empresa, fecha_inicio, fecha_fin):
    ...
    total_repuestos = RepuestoDocumento.objects.filter(
        ...
        valor=Sum(F('cantidad') * F('precio_unitario'))
    )

    total_servicios = LineaServicio.objects.filter(
        documento__in=documentos
    ).aggregate(
