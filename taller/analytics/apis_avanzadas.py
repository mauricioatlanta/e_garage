...
def dashboard_realtime_metrics(request):
    ...
    try:
        ...
        nuevos_suscriptores = Empresa.objects.filter(
            fecha_registro__date=hoy, estado="activa"
        ).count()
...
def dashboard_predictive_analytics(request):
    ...
    try:
        ...
        if len(datos_historicos) >= 2:
            ...
            porcentaje_crecimiento = round(
                ((datos_historicos[-1] - mes_anterior) / mes_anterior) * 100, 1
            )
        else:
        ...
        if porcentaje_crecimiento > 10:
            insights.append(
                "Crecimiento acelerado detectado. Considerar aumentar recursos de soporte."
            )
        ...
        data = {
            ...
            "datos_historicos": {"meses": meses, "suscriptores": datos_historicos},
            "insights": insights,
...
def dashboard_geographic_analysis(request):
    ...
    try:
        ...
        empresas_por_ciudad = (
            Empresa.objects.filter(estado="activa")
            .values("ciudad")
...
def dashboard_alertas_avanzadas(request):
    ...
    try:
        ...
        empresas_vencidas = Empresa.objects.filter(
            fecha_expiracion__lt=hoy, estado="activa"
        ).select_related("usuario")
...
def dashboard_user_behavior(request):
    ...
    try:
        ...
        top_usuarios = (
            Usuario.objects.filter(last_login__gte=hace_30_dias, empresa__isnull=False)
...
def enviar_recordatorio_empresa(request, empresa_id):
    ...
    try:
        ...
        # Log de la acción
        logger.info(
            f"Recordatorio enviado a empresa {empresa.nombre} por usuario {request.user.username}"
        )
    ...
    try:
        ...
        data = {
            ...
            "tasas": {
                "conversion_trial": (
                    round(
                        (empresas_activas / (empresas_trial + empresas_activas) * 100),
                        1,
                    )
                    if (empresas_trial + empresas_activas) > 0
                    else 0
                ),
                "activacion": (
