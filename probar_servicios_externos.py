...
from taller.models import Empresa, Documento
...
def probar_servicios_externos():
    ...
    for servicio in servicios_externos[:3]:  # Mostrar solo los primeros 3
        ...
        print(f"      💰 Costo: ${servicio.costo_taller} | Cliente: ${servicio.precio_cliente} | Ganancia: ${servicio.ganancia}")
    ...
    print(f"💰 Los servicios externos pueden ser agregados a documentos con precios configurados")
    
    # Lista de servicios disponibles
    print(f"\n🏢 SERVICIOS EXTERNOS DISPONIBLES:")
    for i, servicio in enumerate(servicios_externos, 1):
        ganancia_pct = ((servicio.precio_cliente - servicio.costo_taller) / servicio.costo_taller * 100) if servicio.costo_taller > 0 else 0
        print(f"   {i}. {servicio.nombre}")
        print(f"      🏢 Empresa: {servicio.empresa_externa}")
        print(f"      💰 Costo: ${servicio.costo_taller:,.0f} | Cliente: ${servicio.precio_cliente:,.0f}")
