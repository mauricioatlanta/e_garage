...
class ValidadorAvanzado:
    ...
    def validar_8_logica_multiempresa(self):
        ...
        try:
            ...
            docs_cruzados = Documento.objects.filter(
                ...
                empresa=F('vehiculo__cliente__empresa')
            )
            ...
            servicios_por_pais = Servicio.objects.values('country').annotate(
                count=Count('id')
            ).order_by('country')
            
            detalles_paises = ", ".join([f"{s['country']}:{s['count']}" for s in servicios_por_pais])
            self.log_resultado(
    ...
    def validar_9_performance_consultas(self):
        ...
        servicios_cl = Servicio.objects.filter(
            country='CL'
        ).select_related('subcategoria').prefetch_related('names')[:10]
    ...
    def validar_11_integridad_busqueda(self):
        ...
        try:
            ...
            servicios_sin_nombre = Servicio.objects.filter(
                names__isnull=True
            ).count()
