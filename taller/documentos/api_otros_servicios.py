...
def api_buscar_otros_servicios(request):
    ...
    servicios = Servicio.objects.filter(
        subcategoria=subcat, country=country, names__label__icontains=query
    ).distinct()[:20]
    servicios_data = []
...
def api_crear_otro_servicio_rapido(request):
    ...
    if created:
        ServicioName.objects.create(
            servicio=servicio, language="es", label=nombre_es, is_default=True
        )
        ServicioName.objects.create(
            servicio=servicio, language="en", label=nombre_en, is_default=True
        )
        return JsonResponse(
            {
                "success": True,
