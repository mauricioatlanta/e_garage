...
def crear_documentos_mauricio1():
            ...
            try:
                ...
                if servicios and random.choice([True, False]):
                    ...
                    LineaOtroServicio.objects.create(
                        ...
                        empresa_externa=random.choice(["Servicios Externos SA", "Taller Especializado", "Proveedores Unidos"]),
                ...
                print(f"  ✅ Documento {i+1}/10 - {documento.numero_documento} (PK: {documento.pk})")
    ...
    print(f"  - Total repuestos en docs: {LineaRepuesto.objects.filter(documento__empresa=empresa).count()}")
    print(f"  - Total servicios en docs: {LineaServicio.objects.filter(documento__empresa=empresa).count()}")
    print(f"  - Total otros servicios: {LineaOtroServicio.objects.filter(documento__empresa=empresa).count()}")
