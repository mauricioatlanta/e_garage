...
for notif in notificaciones:
    ...
    print(
        f"  - {created}: {notif.tipo_notificacion.nombre} -> {notif.destinatario_nombre} ({notif.estado})"
    )
...
docs_recientes = Documento.objects.filter(
    fecha_emision__gte=timezone.now().date() - timedelta(days=1)
)
