...
from taller.models import (Cliente, Documento, Empresa, Repuesto, Tecnico,
                           Vehiculo)
...
def test_http_documento():
    ...
    documento_data = {
        ...
        'servicios_data': json.dumps([
            {'nombre': 'Cambio aceite', 'cantidad': '1', 'precio': '35000', 'descuento': '0', 'codigo': 'SER-001'}
        ]),
        'otros_servicios_data': json.dumps([
            {'nombre': 'Lavado', 'empresa_externa': 'LavaMax', 'cantidad': '1', 'costo_interno': '8000', 'precio_cliente': '12000'}
        ])
    }
    ...
    if docs_despues > docs_antes:
        ...
        if response.url == lista_url:
            print(f"   ✅ Redirigió correctamente al listado")
        else:
