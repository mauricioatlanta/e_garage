...
class MonitoringSetup:
    ...
    def generar_reporte_uso(self):
        ...
        try:
            ...
            top_acciones = (
                LogAuditoria.objects.filter(timestamp__gte=hace_7_dias)
                .values("accion")
                .annotate(count=models.Count("accion"))
                .order_by("-count")[:5]
            )
    ...
    def verificar_backups(self):
        ...
        if not backup_dir.exists():
            return {"status": "error", "mensaje": "Directorio de backups no existe"}
        ...
        try:
            ...
            if not backups:
                return {"status": "warning", "mensaje": "No se encontraron backups"}
            ...
            return {"status": "error", "mensaje": f"Error verificando backups: {e}"}
