    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if sheet_name not in workbook.sheetnames:
                raise CommandError(
                    f'La hoja "{sheet_name}" no existe. Hojas disponibles: {", ".join(workbook.sheetnames)}'
                )
            ...
            if empresa_id:
                ...
                self.stdout.write(
                    f"   Importando para todas las empresas ({empresas.count()} empresas)"
                )
            ...
            for empresa in empresas:
                ...
                casa, created = CasaRepuestos.objects.get_or_create(
                    empresa=empresa, nombre=casa_nombre, defaults={"activa": True}
                )
                if created:
                    self.stdout.write(
                        self.style.SUCCESS(f'      ✅ Casa de repuestos "{casa_nombre}" creada')
                    )
                else:
                ...
                for idx, row in enumerate(
                    sheet.iter_rows(min_row=1 + skip_rows, values_only=True), start=1 + skip_rows
                ):
                    ...
                    try:
                        ...
                        nombre = (
                            ...
                            else part_number or "Sin nombre"
                        )
                        ...
                        try:
                            if isinstance(precio_str, (int, float)):
                                ...
                                precio_str = (
                                    str(precio_str).replace("$", "").replace(",", "").strip()
                                )
                                precio_referencia = (
                                    Decimal(precio_str) if precio_str else Decimal("0.00")
                                )
                        except (ValueError, TypeError):
                            self.stdout.write(
                                self.style.WARNING(
                                    f'      ⚠️  Fila {idx}: Precio inválido "{precio_str}", usando 0'
                                )
                            )
                        ...
                        with transaction.atomic():
                            ...
                            if created:
                                ...
                                if total_creados % 10 == 0:
                                    self.stdout.write(
                                        f"         ✅ {total_creados} productos creados..."
                                    )
                            elif update:
                                ...
                                if total_actualizados % 10 == 0:
                                    self.stdout.write(
                                        f"         🔄 {total_actualizados} productos actualizados..."
                                    )

                    except Exception as e:
                        total_errores += 1
                        self.stdout.write(
                            self.style.ERROR(f"      ❌ Error en fila {idx}: {str(e)}")
                        )
                        continue
