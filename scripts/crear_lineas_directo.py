...
try:
    ...
    if lineas_rep == 0:
        ...
        print(f"✅ Repuestos creados:")
        print(f"   - {linea1.nombre}: {linea1.cantidad} x ${linea1.precio_unitario} = ${linea1.subtotal}")
        print(f"   - {linea2.nombre}: {linea2.cantidad} x ${linea2.precio_unitario} (desc: {linea2.descuento}%) = ${linea2.subtotal}")
    ...
    if lineas_serv == 0:
        ...
        print(f"✅ Servicios creados:")
        print(f"   - {linea3.nombre}: {linea3.cantidad} x ${linea3.precio_unitario} = ${linea3.subtotal}")
        print(f"   - {linea4.nombre}: {linea4.cantidad} x ${linea4.precio_unitario} (desc: {linea4.descuento}%) = ${linea4.subtotal}")
    ...
    if total_rep > 0 or total_serv > 0:
        print(f"\n🎉 ¡ÉXITO! Recargar:")
        print(f"   - US: http://127.0.0.1:8000/us/documentos/us/")
        print(f"   - CL: http://127.0.0.1:8000/cl/documentos/cl/")
    else:
