...
def generar_resumen_final():
    ...
    print(f"🏢 Empresas demo: {empresas_demo.count()} (CL: {empresas_cl.count()}, US: {empresas_us.count()})")
    print(f"👥 Clientes: {clientes_demo.count()}")
    print(f"🚗 Vehículos: {vehiculos_demo.count()}")
    print(f"🔧 Servicios: {servicios_demo.count()} (CL: {servicios_cl.count()}, US: {servicios_us.count()})")
    ...
    for empresa in empresas_demo:
        ...
        print(f"   {empresa.nombre_taller} ({empresa.pais}): {clientes_emp} clientes, {vehiculos_emp} vehículos, {documentos_emp} docs")
