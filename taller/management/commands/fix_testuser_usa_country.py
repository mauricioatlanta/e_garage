        ...
        try:
            with transaction.atomic():
                ...
                for empresa in empresas_user:
                    self.stdout.write(
                        f"  ANTES: {empresa.nombre_taller} -> {empresa.pais}"
                    )
                    empresa.pais = "US"
                    empresa.save()
                    empresa.refresh_from_db()
                    self.stdout.write(
                        f"  DESPUÉS: {empresa.nombre_taller} -> {empresa.pais}"
                    )

                self.stdout.write(
                    f"Empresas con campo usuario: {empresas_usuario.count()}"
                )
                ...
                # Verificación final
                self.stdout.write("\n=== VERIFICACIÓN FINAL ===")
                all_empresas = Empresa.objects.filter(
                    user=user
                ) | Empresa.objects.filter(usuario=user)
                for empresa in all_empresas:
                    self.stdout.write(
                        f"Empresa: {empresa.nombre_taller} -> País: {empresa.pais}"
                    )

                if all_empresas.filter(pais="US").count() > 0:
                    self.stdout.write(
                        self.style.SUCCESS(
                            "✅ ¡Cambio completado! Al menos una empresa tiene país US"
                        )
                    )
                else:
                    self.stdout.write(
