...
def main():
    ...
    try:
        ...
        with connection.cursor() as cursor:
            cursor.execute("SELECT id, nombre_taller, pais, user_id, usuario_id FROM taller_empresa WHERE user_id = %s OR usuario_id = %s", [user.id, user.id])
            rows = cursor.fetchall()
            print(f"\n✓ Verificación SQL directa:")
            for row in rows:
                print(f"  - ID: {row[0]}, Nombre: {row[1]}, País: {row[2]}, user_id: {row[3]}, usuario_id: {row[4]}")
