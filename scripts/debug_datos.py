...
for doc in docs:
    print(
        f'Doc ID {doc.id}: {doc.numero_documento} - {doc.tipo_documento} - Empresa: {doc.empresa.id if doc.empresa else "Sin empresa"}'
    )
...
for rep in repuestos:
    print(
        f"Repuesto ID {rep.id}: Doc {rep.documento_id} - {rep.nombre} - Qty: {rep.cantidad} - Precio: {rep.precio}"
    )
