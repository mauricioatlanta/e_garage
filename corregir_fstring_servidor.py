...
pattern1 = r'f"https://wa\.me/\{telefono\}\?text=\{mensaje\.replace\([^)]+\)\.replace\([^)]*\\n[^)]+\)\}"'
...
if re.search(pattern1, content):
    ...
    while i < len(lines):
        ...
        if 'wa.me' in line and 'mensaje.replace' in line and '\\n' in line:
            ...
            start_idx = i
            while start_idx > 0 and not lines[start_idx].strip().startswith('def ') and not lines[start_idx].strip().startswith('@'):
                start_idx -= 1
