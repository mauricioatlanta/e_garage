...
def changelog_view(request):
    cambios = [
        {"version": "v1.0.0-beta", "fecha": "2025-08-14", "items": [
...
from .forms.empresa import EmpresaForm, DatosPersonalesForm
    ...
    if request.method == 'POST':
        if 'datos_form' in request.POST:
            ...
            if len(nombre) < 2:
                ...
                Tecnico.objects.create(empresa=empresa, nombre=nombre, telefono=telefono, direccion=direccion)
            ...
            if tec:
                ...
                tec.save()
                messages.success(request, f'Técnico "{tec.nombre}" ahora está {"activo" if tec.activo else "inactivo"}.')
            else:
