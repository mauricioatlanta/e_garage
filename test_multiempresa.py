...
def test_multiempresa_filtrado():
    ...
    clientes_por_empresa = sum(
        Cliente.objects.filter(empresa=emp).count() for emp in empresas
    )
    vehiculos_por_empresa = sum(
        Vehiculo.objects.filter(empresa=emp).count() for emp in empresas
    )

    print(f"   📊 Clientes totales: {total_clientes}")
    print(f"   📊 Clientes por empresa: {clientes_por_empresa}")
    print(
        f"   {'✅' if total_clientes == clientes_por_empresa else '❌'} Consistencia clientes"
    )

    print(f"   📊 Vehículos totales: {total_vehiculos}")
    print(f"   📊 Vehículos por empresa: {vehiculos_por_empresa}")
    print(
        f"   {'✅' if total_vehiculos == vehiculos_por_empresa else '❌'} Consistencia vehículos"
    )
    ...
    print(f"\n🎯 ESTADO FINAL:")
    if (
        total_clientes == clientes_por_empresa
        and total_vehiculos == vehiculos_por_empresa
    ):
...
def crear_datos_test_multiempresa():
    ...
    for i, user in enumerate(usuarios):
        ...
        try:
            ...
            empresa, created = Empresa.objects.get_or_create(
                usuario=user, defaults={"nombre_taller": f"Taller {user.username}"}
            )
    ...
    print(
        f"   📊 Clientes antes: {contador_clientes}, después: {Cliente.objects.count()}"
    )
