...
def test_vehiculo_unique_empresa_patente_db_constraint():
    ...
    User = get_user_model()
    emp = Empresa.objects.create(nombre_taller="UQ", pais="CL", user=User.objects.create_user(username="emp_uq", password="x"))
    cli = Cliente.objects.create(empresa=emp, nombre="Juan", tax_id="1-9")
    Vehiculo.objects.create(empresa=emp, cliente=cli, patente="UQ1234", marca_texto="A", modelo_texto="B", anio=2021)
