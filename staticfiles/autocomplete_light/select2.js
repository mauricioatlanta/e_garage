...
document.addEventListener('dal-init-function', function () {
    if (yl.functions.hasOwnProperty('select2')) {
        console.warn('The DAL function "select2" is already registered. Skipping initialization.');
        return; // Evita inicializar nuevamente
    }

    yl.registerFunction('select2', function ($, element) {

        // CUSTOM PATCH: Forzar consulta AJAX al abrir el campo si minimumInputLength=0
        var $element = $(element);
        $element.on('select2:open', function(e) {
            var minLen = parseInt($element.attr('data-minimum-input-length') || '0');
            if (minLen === 0) {
