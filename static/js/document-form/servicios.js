...
(function() {
    ...
    function addServicioRow(presetRowId) {
        ...
        row.innerHTML = ''
            + '<div class="doc-row-flex flex items-end gap-2 border border-cyan-400/30 rounded-lg p-1 bg-black/20 w-full" data-row-id="' + rowId + '">'
            + '  <div class="flex-1 description-field srv-input-container relative" style="min-width:900px;max-width:none;width:100%;">'
            + '    <label class="block text-cyan-200 text-sm mb-1">' + escapeHtml(EG.I18N.service || 'Service') + '</label>'
            + '    <div class="flex gap-1 items-center w-full" style="width:100%;">'
            ...
            + '      <button type="button" class="srv-create-btn btn-add w-8 flex-shrink-0 h-9 px-0" title="' + escapeHtml(EG.I18N.create_service || 'Create Service') + '">+</button>'
            + '    </div>'
    ...
    function addOtroServicioRow(presetRowId) {
        ...
        row.innerHTML = ''
            + '<div class="doc-row-grid grid grid-cols-12 gap-1 items-center border border-cyan-400/30 rounded-lg p-1 bg-black/20">'
