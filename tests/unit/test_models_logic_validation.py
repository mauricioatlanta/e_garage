...
def test_lineas_documento_logic():
    ...
    test_cases = [
        ...
        (Decimal("1.5"), Decimal("1000.00"), Decimal("0.00"), Decimal("1500.00")),  # decimal quantity
    ]
    ...
    for i, (cantidad, precio_unitario, descuento, expected_subtotal) in enumerate(test_cases):
        ...
        if hasattr(linea, 'subtotal'):
            ...
            assert isinstance(linea.subtotal, (int, float, Decimal)), f"Failed for case {i}: subtotal should be numeric, got {type(linea.subtotal)}"
            assert linea.subtotal >= 0, f"Failed for case {i}: subtotal should be non-negative, got {linea.subtotal}"
    ...
    try:
        ...
        if hasattr(linea, 'subtotal'):
            # Just verify that subtotal is calculated and is a valid number
            assert isinstance(linea.subtotal, (int, float, Decimal)), f"subtotal should be numeric, got {type(linea.subtotal)}"
