...
class Command(BaseCommand):
    ...
    def handle(self, *args, **opts):
        ...
        self.stdout.write(f"🏢 Empresa: {getattr(empresa, 'nombre_taller', 'None')} (id={getattr(empresa, 'id', 'None')})")
        ...
        if all_empresas.count() > 1:
            self.stdout.write(self.style.WARNING(f"⚠️  Usuario tiene {all_empresas.count()} empresas:"))
