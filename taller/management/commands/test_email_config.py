...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(f"  EMAIL_USE_SSL: {getattr(settings, 'EMAIL_USE_SSL', 'NO CONFIGURADO')}")
        ...
        if email_password:
            self.stdout.write(f"  EMAIL_HOST_PASSWORD: {'*' * len(email_password)} ({len(email_password)} caracteres)")
        else:
        ...
        self.stdout.write(f"  ACCOUNT_EMAIL_VERIFICATION: {getattr(settings, 'ACCOUNT_EMAIL_VERIFICATION', 'NO CONFIGURADO')}")
        self.stdout.write(f"  ACCOUNT_EMAIL_REQUIRED: {getattr(settings, 'ACCOUNT_EMAIL_REQUIRED', 'NO CONFIGURADO')}")
        self.stdout.write("")
        ...
        recipient = options.get('recipient') or getattr(settings, 'ADMIN_EMAIL', 'subscription@egarage.cl')
