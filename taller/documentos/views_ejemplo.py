...
def documento_crear(request):
    ...
    context = {
        ...
        } if empresa else {"simbolo": "$", "codigo": "CLP", "decimales": 0},
...
def documento_editar(request, pk):
    ...
    if request.method == "POST":
        form = DocumentoForm(request.POST, instance=obj, user=request.user, empresa=empresa, country=country)
        if form.is_valid():
            form.save()
...
def documento_ver_pdf(request, pk):
    ...
    context = {
        ...
        "verification_code": f"DOC-{obj.id}-{obj.fecha_emision.strftime('%Y%m%d')}" if obj.fecha_emision else f"DOC-{obj.id}",
