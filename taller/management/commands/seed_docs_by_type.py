    ...
    if not veh:
        ...
        for fld in ("millas", "kilometraje", "odometro"):
            if hasattr(Vehiculo, fld):
                ...
                break
        veh = Vehiculo.objects.create(empresa=emp, cliente=cli, vin="1FAFP404XWF123456", **veh_defaults)
...
def add_lines(doc: Documento, tec: Tecnico, min_items=3):
    emp = doc.empresa
    # Repuestos (>=3)
    all_parts = list(Repuesto.objects.filter(empresa=emp).values_list("id","part_number","nombre","precio_venta"))
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("--per-type", type=int, default=10, help="Documentos por tipo (default 10)")
