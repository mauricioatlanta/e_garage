...
def test_crear_vehiculo_requires_auth_then_ok(client):
    ...
    # anónimo
    r = client.post(url, data='{"empresa_id": 1, "cliente_id": 1, "patente": "TEST", "marca": "X", "modelo": "Y"}', content_type="application/json")
    assert r.status_code in (302, 400, 401, 403, 405)  # redirige a login, valida datos o bloquea
    ...
    emp = Empresa.objects.create(nombre_taller="Auth", pais="CL", user=User.objects.create_user(username="emp_auth", password="x"))
    cli = Cliente.objects.create(empresa=emp, nombre="Ana", tax_id="1-9")
    payload = {
        "empresa_id": emp.id, "cliente_id": cli.id,
        "patente": "AUTH01", "marca": "X", "modelo": "Y", "anio": 2020
    }
