...
def verificar_conteos():
    ...
    ).order_by('-fecha_creacion')[:5]  # Solo los últimos 5 para verificar
    
    print("ÚLTIMOS 5 DOCUMENTOS CON CONTEOS:")
    print("-" * 80)
    print(f"{'ID':<5} {'Fecha':<12} {'Millas':<8} {'#Rep':<6} {'#Serv':<7} {'#Otros':<8} {'Total $':<10}")
    print("-" * 80)
    ...
    for doc in documentos:
        ...
        print(f"{doc.id:<5} {fecha_str:<12} {doc.total_millas or 0:<8} "
    ...
    for doc in documentos[:3]:  # Solo los primeros 3 para detalle
        ...
        millas_directo = VehiculoDocumento.objects.filter(documento=doc).aggregate(
        ...
        if diferencias:
            ...
            print(f"  ✅ CONTEOS CORRECTOS")
        print()
    
    # Estadísticas generales
    total_docs = Documento.objects.count()
    docs_con_vehiculos = Documento.objects.filter(vehiculodocumento__isnull=False).distinct().count()
    docs_con_repuestos = Documento.objects.filter(repuestodocumento__isnull=False).distinct().count()
    docs_con_servicios = Documento.objects.filter(serviciodocumento__isnull=False).distinct().count()
    docs_con_otros = Documento.objects.filter(servicioexternodocumento__isnull=False).distinct().count()
