    ...
    def handle(self, *args, **options):
        ...
        for repuesto in Repuesto.objects.filter(empresa=empresa):
            ...
            for nombre_patron, (min_precio, max_precio) in precios_repuestos.items():
                if nombre_patron.lower() in repuesto.nombre.lower():
                    precio_compra = Decimal(str(RND.randint(int(min_precio * 0.6), int(min_precio * 0.8))))
        ...
        for servicio in Servicio.objects.filter(empresa=empresa):
            if hasattr(servicio, 'precio_base'):
                # Buscar precio baseado en el nombre
                precio_base = None
                for nombre_patron, (min_precio, max_precio) in precios_servicios.items():
        ...
        # ==========================================
        total_corregidos = (
            repuestos_corregidos + servicios_corregidos + 
            lineas_repuestos_corregidas + lineas_servicios_corregidas + 
        ...
        if dry_run:
            self.stdout.write(
                self.style.WARNING(f'🔍 MODO SIMULACIÓN: Se habrían corregido {total_corregidos} precios')
            )
        else:
            self.stdout.write(
                self.style.SUCCESS(f'✅ CORRECCIÓN COMPLETADA: {total_corregidos} precios corregidos')
            )
