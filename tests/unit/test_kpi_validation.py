...
from taller.models import Empresa, Tecnico, Cliente, Vehiculo, Documento, LineaServicio
...
def test_kpi_tecnico_mes_actual_consistency():
    ...
    manual_results = list(
        ...
        .filter(documento__fecha_emision__month=timezone.now().month)
    ...
    if kpi_results and manual_results:
        ...
        assert abs(kpi_total - manual_total) < Decimal("0.01"), f"Totales diferentes: {kpi_total} vs {manual_total}"
        
        # Verificar que el técnico es el mismo
        assert kpi_results[0]['documento__tecnico_responsable__nombre'] == manual_results[0]['documento__tecnico_responsable__nombre']
...
def test_kpi_calculator_consistency():
    ...
    cliente = Cliente.objects.create(empresa=empresa, nombre="Test Client 2", rut_ein="12.345.678-9")
    vehiculo = Vehiculo.objects.create(
        empresa=empresa, 
        cliente=cliente, 
        patente="TEST456", 
...
def test_kpi_documentos_por_estado_consistency():
    ...
    manual_results = list(
        Documento.objects
        .filter(fecha_emision__month=timezone.now().month)
...
def test_kpi_helpers_monto_calculado():
    ...
    cliente = Cliente.objects.create(empresa=empresa, nombre="Test Client 4", rut_ein="12.345.678-9")
    vehiculo = Vehiculo.objects.create(
        empresa=empresa, 
        cliente=cliente, 
        patente="TEST000", 
    ...
    assert abs(resultado.monto - monto_esperado) < Decimal("0.01"), f"Monto calculado incorrecto: {resultado.monto} vs {monto_esperado}"
