...
def test_protected_views_bulk():
    ...
    for names, fb in CANDIDATES:
        ...
        r_anon = anon.get(url)
        assert r_anon.status_code in (200, 301, 302, 401, 403, 404, 405), f"{url} inesperado anónimo"
        ...
        try:
            r_auth = auth.get(url)
            assert r_auth.status_code in (200, 301, 302, 401, 403, 404, 405, 500), f"{url} inesperado auth"
        except Exception:
