...
class LanguagePolicyMiddleware:
    ...
    def __call__(self, request):
        try:
            ...
            if pais == "US":
                # Signup público USA: idioma fijo por URL (no arrastrar cookie de /cl/ u otros).
                path_norm = (request.path or "").rstrip("/") or "/"
                if path_norm == "/us/signup":
                    lang = "en"
                    request.session[DJANGO_LANGUAGE_SESSION_KEY] = "en"
                    request.eg_us_public_signup_lang = "en"
                elif path_norm == "/us/es/signup":
                    lang = "es"
