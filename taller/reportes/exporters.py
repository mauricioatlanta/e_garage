...
def generar_pdf_mecanico(mecanico_id, fecha_desde=None, fecha_hasta=None):
    ...
    documentos = Documento.objects.filter(mecanico=mecanico, fecha__range=[fecha_desde, fecha_hasta])
...
def exportar_csv_personalizado(mecanico_id=None, fecha_desde=None, fecha_hasta=None):
    ...
    documentos_qs = Documento.objects.filter(fecha__range=[fecha_desde, fecha_hasta], mecanico__isnull=False)
    if mecanico_id:
    ...
    writer.writerow([
        'Fecha', 'Mecánico', 'Tipo Documento', 'Número Documento', 'Cliente', 'Vehículo', 'Servicios', 'Total Generado', 'Observaciones'
    ])

    for documento in documentos_qs:
        servicios = LineaServicio.objects.filter(documento=documento)
        total = servicios.aggregate(total=Sum(F('precio_unitario') * F('cantidad'))).get('total') or 0
        ...
        writer.writerow([
            ...
            documento.mecanico.nombre if getattr(documento, 'mecanico', None) else '',
            getattr(documento, 'tipo_documento', ''),
            getattr(documento, 'numero_documento', '') or '',
            str(getattr(documento, 'cliente', '')) if getattr(documento, 'cliente', None) else '',
            f"{documento.vehiculo.patente} - {documento.vehiculo.modelo}" if getattr(documento, 'vehiculo', None) else '',
            servicios_nombres,
            total,
...
def generar_estadisticas_avanzadas(fecha_desde=None, fecha_hasta=None):
    ...
    for mecanico in Tecnico.objects.all():
        ...
        total_generado = servicios_mecanico.aggregate(total=Sum(F('precio_unitario') * F('cantidad'))).get('total') or 0
...
class ReporteMecanicoWhatsApp:
    ...
    def generar_resumen_semanal(mecanico_id):
        ...
        documentos = Documento.objects.filter(mecanico=mecanico, fecha__range=[fecha_desde, fecha_hasta])
        servicios = LineaServicio.objects.filter(documento__in=documentos)
        total_generado = servicios.aggregate(total=Sum(F('precio_unitario') * F('cantidad'))).get('total') or 0

        servicios_top = servicios.values('nombre').annotate(cantidad=Count('id')).order_by('-cantidad')[:3]
        ...
        mensaje.extend([f"{i+1}. {s['nombre']} ({s['cantidad']}x)" for i, s in enumerate(servicios_top)])
        mensaje.append("")
    ...
    def generar_resumen_mensual(mecanico_id):
        ...
        documentos_anterior = Documento.objects.filter(mecanico=mecanico, fecha__range=[fecha_mes_anterior_inicio, fecha_mes_anterior_fin])
        servicios_anterior = LineaServicio.objects.filter(documento__in=documentos_anterior)
        total_anterior = servicios_anterior.aggregate(total=Sum(F('precio_unitario') * F('cantidad'))).get('total') or 0
