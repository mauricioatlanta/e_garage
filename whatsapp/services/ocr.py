...
# Intentar importar EasyOCR y OpenCV
try:
    import easyocr
    import cv2
    ...
    logger.warning("EasyOCR no está instalado. Ejecuta: pip install easyocr opencv-python-headless")
...
def get_reader(languages=None, gpu=False):
    ...
    if (_READER_INSTANCE is None or 
        _READER_LANGUAGES != languages or 
        _READER_GPU != gpu):
        
        try:
            logger.info(f"Inicializando EasyOCR Reader (Singleton) - idiomas: {languages}, GPU: {gpu}")
...
class OCRProcessor:
    ...
    def __init__(self, languages=None, gpu=False, confidence_threshold=0.6):
        ...
        if not EASYOCR_AVAILABLE:
            logger.error("EasyOCR no está disponible. Ejecuta: pip install easyocr opencv-python-headless")
    
    def extract_plate(self, image_bytes: bytes, return_full_result: bool = False) -> Optional[Dict[str, Any]]:
        """
        ...
        if not image_bytes:
            ...
            return None if not return_full_result else {
                'plate': None,
                'confidence': 0.0,
        ...
        try:
            ...
            if needs_confirm:
                logger.warning(f"Confianza baja ({best_confidence:.2f}) - requiere confirmación manual: {best_candidate['plate']}")
            else:
                logger.info(f"Patente detectada con confianza {best_confidence:.2f}: {best_candidate['plate']}")
    ...
    def _find_all_plates_in_text(self, text_items: list) -> list:
        ...
        combined_clean = re.sub(r'[^\w\s-]', '', combined_text.upper()).replace(' ', '').replace('-', '')
        
        # Buscar en cada texto individual y en el texto combinado
        search_texts = [
            {'text': combined_clean, 'confidence': max([item['confidence'] for item in text_items], default=0.0)}
        ]
        search_texts.extend([
            {'text': re.sub(r'[^\w\s-]', '', item['text'].upper()).replace(' ', '').replace('-', ''), 
                    ...
                    if match:
                        ...
                        if plate and not any(c['plate'] == plate for c in candidates):
                            candidates.append({
                                ...
                                'priority': 1 if is_strict else 0  # Mayor prioridad si match estricto
                            })
        ...
        for country in countries_to_try:
            ...
            for pattern in patterns:
                ...
                if match:
                    ...
                    if normalized:
                        logger.info(f"Patente encontrada ({country}) con patrón {pattern}: {normalized}")
                        return normalized
                ...
                for match_text in matches:
                    ...
                    if normalized:
                        logger.info(f"Patente encontrada ({country}) parcial con patrón {pattern}: {normalized}")
                        return normalized
