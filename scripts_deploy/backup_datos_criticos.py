...
def hacer_backup_tabla(cursor, nombre_tabla, backup_dir):
    ...
    try:
        ...
        archivo_backup = (
            backup_dir / f"{nombre_tabla}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
...
def hacer_backup_completo():
    ...
    # Convertir todos los Path objects a strings para JSON
    resumen = {
        "timestamp": datetime.now().isoformat(),
        "base_datos": str(db_path) if db_path else None,
...
def hacer_backup_sql_completo():
    ...
    # Intentar usar sqlite3 del sistema primero (Linux/Mac)
    import subprocess
    import platform

    # En Windows, intentar usar Python directamente
    if platform.system() == "Windows":
        try:
            # Usar Python con sqlite3 para hacer dump
            conn = sqlite3.connect(db_path)
            ...
            print(f"⚠️  No se pudo crear backup SQL usando Python: {e}")
            print("   (El backup JSON ya está completo y es suficiente)")
            return None
    else:
        # En Linux/Mac, intentar usar comando sqlite3
        try:
            with open(sql_backup, "w", encoding="utf-8") as f:
                result = subprocess.run(
                    ["sqlite3", str(db_path), ".dump"], stdout=f, stderr=subprocess.PIPE, text=True
                )
            ...
            if result.returncode == 0:
                ...
                print(f"⚠️  Error al crear backup SQL con sqlite3: {result.stderr}")
                print("   Intentando con Python...")
                # Fallback a Python
                conn = sqlite3.connect(db_path)
                with open(sql_backup, "w", encoding="utf-8") as f:
                    for line in conn.iterdump():
                        f.write(f"{line}\n")
                conn.close()
                print(f"✅ Backup SQL completo creado con Python: {sql_backup}")
                return sql_backup
        except FileNotFoundError:
            # sqlite3 no está instalado, usar Python
            print("⚠️  sqlite3 no está disponible, usando Python...")
            try:
                conn = sqlite3.connect(db_path)
                with open(sql_backup, "w", encoding="utf-8") as f:
                    for line in conn.iterdump():
                ...
                print(f"⚠️  No se pudo crear backup SQL: {e}")
                print("   (El backup JSON ya está completo y es suficiente)")
                return None
        except Exception as e:
            print(f"⚠️  Error al crear backup SQL: {e}")
            print("   (El backup JSON ya está completo y es suficiente)")
            return None
