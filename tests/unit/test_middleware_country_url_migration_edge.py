...
def test_country_url_migration_edge_cases():
    ...
    test_cases = [
        ...
        ("/us/vehiculos/?next=/cl/documentos/", "/cl/vehiculos/?next=/cl/documentos/", "With next parameter"),
        ("/us/vehiculos?next=/cl/documentos/", "/cl/vehiculos/?next=/cl/documentos/", "With next parameter (no trailing slash)"),
        ("/us/vehiculos/?search=test&page=2", "/cl/vehiculos/?search=test&page=2", "With multiple query params"),
    ...
    for path, expected_redirect, description in test_cases:
        ...
        if expected_redirect is None:
            ...
            assert response.url == expected_redirect, f"Failed for {description}: expected {expected_redirect}, got {response.url}"
    ...
    for path in complex_cases:
        ...
        if '?' in redirect_url:
            ...
            if 'search=' in original_querystring:
                assert 'search=' in querystring, f"Search parameter should be preserved in {redirect_url}"
...
def test_country_url_migration_no_500_errors():
    ...
    for path in edge_cases:
        try:
            ...
            assert response is None or (hasattr(response, 'status_code') and response.status_code < 500), \
