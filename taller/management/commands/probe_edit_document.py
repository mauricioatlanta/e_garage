...
class Command(BaseCommand):
    ...
    def handle(self, *args, **opts):
        ...
        for doc in qs:
            ...
            self.stdout.write(
                f"[DOC {doc.pk}] Probando edición de {doc.tipo}-{doc.numero} (empresa: {doc.empresa.nombre_taller if doc.empresa else 'N/A'})"
            )
            ...
            ok = (status in (302, 200)) and (
                "Corrige los errores" not in (resp.content.decode(errors="ignore"))
            )
