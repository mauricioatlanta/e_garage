...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("backup_final.log"), logging.StreamHandler()],
)
...
class BackupFinal:
    ...
    def crear_backup_empresa(self, empresa):
        ...
        try:
            ...
            from taller.models.lineas_documento import (
                LineaServicio as ServicioDocumento,
            )
            ...
            if documentos_empresa.exists():
                try:
                    ...
                    repuestos_doc_count = RepuestoDocumento.objects.filter(
                        documento__in=documentos_empresa
                    ).count()
                    servicios_doc_count = ServicioDocumento.objects.filter(
                        documento__in=documentos_empresa
                    ).count()
