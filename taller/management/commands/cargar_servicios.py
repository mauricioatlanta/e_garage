...
from taller.servicios.models import CategoriaServicio, CategoriaServicioName, SubcategoriaServicio, SubcategoriaServicioName, Servicio, ServicioName
	...
	def handle(self, *args, **options):
		...
		for country, lang, label_key in [('CL', 'es', 'es'), ('US', 'en', 'en')]:
			...
			for cat in categorias:
				...
				CategoriaServicioName.objects.get_or_create(categoria=cat_obj, language=lang, label=cat[label_key], is_default=True)
				# Subcategorías
				for sub in cat['subcategorias']:
					sub_obj, _ = SubcategoriaServicio.objects.get_or_create(categoria=cat_obj, code=sub['code'], country=country)
					SubcategoriaServicioName.objects.get_or_create(subcategoria=sub_obj, language=lang, label=sub[label_key], is_default=True)
					# Servicios
					for serv in sub['servicios']:
						serv_obj, _ = Servicio.objects.get_or_create(subcategoria=sub_obj, code=serv[label_key][:48], country=country)
						ServicioName.objects.get_or_create(servicio=serv_obj, language=lang, label=serv[label_key], is_default=True)

			# Otros servicios
			for cat in otros_servicios:
				cat_obj, _ = CategoriaServicio.objects.get_or_create(country=country, code=cat['code'])
				CategoriaServicioName.objects.get_or_create(categoria=cat_obj, language=lang, label=cat[label_key], is_default=True)
				sub_obj, _ = SubcategoriaServicio.objects.get_or_create(categoria=cat_obj, code=cat['code'], country=country)
				SubcategoriaServicioName.objects.get_or_create(subcategoria=sub_obj, language=lang, label=cat[label_key], is_default=True)
				for serv in cat['servicios']:
					serv_obj, _ = Servicio.objects.get_or_create(subcategoria=sub_obj, code=serv[label_key][:48], country=country)
					ServicioName.objects.get_or_create(servicio=serv_obj, language=lang, label=serv[label_key], is_default=True)

		self.stdout.write(self.style.SUCCESS('Servicios y otros servicios cargados correctamente para Chile y USA.'))
