...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if username:
            ...
            if not users.exists():
                self.stdout.write(self.style.WARNING("⚠️  No se encontraron usuarios con CompanySettings"))
                return
        ...
        for user in users:
            ...
            try:
                ...
                if company_settings:
                    self.stdout.write(self.style.SUCCESS(f"   ✅ CompanySettings encontrado (ID: {company_settings.id})"))
                        ...
                        try:
                            ...
                            if hasattr(company_settings.logo, 'path'):
                                ...
                                if os.path.exists(logo_path):
                                    ...
                                    self.stdout.write(self.style.ERROR(f"   ❌ Archivo NO existe físicamente"))
                            ...
                            self.stdout.write(self.style.ERROR(f"   ❌ Error obteniendo URL del logo: {e}"))
                    else:
        ...
        self.stdout.write("   2. Si CompanySettings existe pero NO tiene logo, sube un logo desde Settings")
