...
def enviar_documento_whatsapp(request, documento_id):
    ...
    try:
        ...
        vehiculo_info = (
            f"{documento.vehiculo.marca} {documento.vehiculo.modelo}"
...
¡Gracias por confiar en nosotros!"""
        ...
        enlace_whatsapp = WhatsAppSender.generar_enlace_whatsapp(
            documento.cliente.telefono, mensaje
        )
...
def enviar_documento_email(request, documento_id):
    ...
    try:
        ...
        if not email_destinatario:
            return JsonResponse(
                {"success": False, "error": "No se ha proporcionado email de destino"}
            )

        # Enviar email
        resultado = EmailSender.enviar_documento_por_email(
            documento, email_destinatario, adjuntar_pdf=incluir_pdf
        )
