...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not documentos_a_revisar:
            self.stdout.write(self.style.SUCCESS("✅ No hay documentos con total=0 que tengan líneas."))
            return
        ...
        for doc, total_lineas in documentos_a_revisar:
            try:
                ...
                actualizados += 1
                self.stdout.write(
                    f"  ✓ ID {doc.id} | total actualizado a {doc.total}"
                )
            except Exception as e:
                errores += 1
                self.stdout.write(
                    self.style.ERROR(f"  ✗ ID {doc.id} | Error: {e}")
                )
