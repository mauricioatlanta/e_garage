...
def company_settings_view(request):
    ...
    if request.method == "POST":
        if "crear_usuario_sistema" in request.POST:
            ...
            rol_post = request.POST.get("rol", "VENDEDOR").strip().upper()
            ...
            rol = role_map.get(rol_post, "Vendedor")
        ...
        if "toggle_usuario_sistema" in request.POST:
            ...
            try:
                ...
                member.save(update_fields=["is_active", "fecha_actualizacion"])
                estado = "activado" if member.is_active else "desactivado"
                messages.success(
                    request,
                    f"Acceso del usuario '{member.user.username}' {estado} correctamente.",
                )
    ...
    # El formulario de dominio comparte visualmente settingsForm.
    # No emitir HTML5 "required" aquí porque bloquearía Guardar Configuración
    # cuando la pestaña Dominio está vacía. La validación backend sigue siendo
    # obligatoria cuando se procesa la acción registrar_dominio.
    form_dominio = DominioPersonalizadoForm(use_required_attribute=False)
