...
from taller.servicios.models import Servicio, ServicioExterno, CategoriaServicio
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            type=int, 
            default=1, 
            help="Scale factor for data volume (default: 1)"
        )
    ...
    def handle(self, *args, **options):
        ...
        categorias_nombres = ["Engine", "Brake", "Electrical", "Suspension", "Filters", "Fluids"]
        categorias = []
        ...
        tipos_repuestos = [
            ...
            "Brake Pads", "Brake Rotors", "Brake Fluid", "Spark Plugs",
            "Timing Belt", "Serpentine Belt", "Battery", "Alternator",
            "Starter", "Radiator", "Thermostat", "Water Pump",
            "Fuel Pump", "Oxygen Sensor", "Mass Air Flow Sensor",
            "Transmission Fluid", "Power Steering Fluid", "Coolant",
            "Shock Absorbers", "Struts", "Ball Joints", "Tie Rods"
        ]
        
        for i in range(n_repuestos):
            nombre_base = random.choice(tipos_repuestos)
            marca_repuesto = random.choice(["OEM", "Bosch", "Denso", "ACDelco", "Motorcraft", "Gates", "Fram"])
            
            precio_venta = Decimal(random.randrange(1500, 45000)) / 100  # USD 15–450
            precio_compra = (precio_venta * Decimal(random.uniform(0.5, 0.8))).quantize(Decimal("0.01"))
        ...
        empresas_externas = [
            "QuickLube Pro", "AutoGlass Experts", "Tire World", "Paint & Body Shop",
            "Transmission Specialists", "AC Repair Center", "Towing Services"
        ]
        ...
        self.stdout.write(f"📄 Creating {total_docs} documents ({n_facturas} invoices + {n_estimados} estimates)...")
        ...
        servicios_comunes = [
            "Oil Change Service", "Brake Pad Replacement", "Coolant System Flush",
            "Transmission Service", "AC System Check", "Battery Replacement",
            "Tire Rotation", "Wheel Alignment", "Engine Diagnostic",
            "Fuel System Cleaning", "Air Filter Replacement", "Spark Plug Service"
        ]
        ...
        for i, tipo in enumerate(tipos_documentos):
            ...
            vehiculo = random.choice(vehiculos_cliente) if vehiculos_cliente else random.choice(vehiculos)
            
            # Crear documento de manera básica evitando campos problemáticos
            try:
                # Generar número secuencial manualmente
                last_doc = Documento.objects.filter(empresa=empresa, tipo=tipo).order_by('-numero').first()
                ...
                with connection.cursor() as cursor:
                    ...
                    """, (
                        tipo, numero, "EMITIDO", fecha_emision.isoformat(), "USD", "US",
                        float(documento.neto_repuestos), float(documento.neto_servicios), 
                        float(documento.descuento), float(documento.tax_rate_applied), 
                        float(documento.tax_amount), float(documento.total),
                        cliente.pk, empresa.pk, vehiculo.pk if vehiculo else None
                    ))
        ...
        self.stdout.write(f"📄 Documents: {total_docs} ({n_facturas} invoices + {n_estimados} estimates)")
