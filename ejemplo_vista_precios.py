...
def vista_precios_ejemplo(request):
    ...
    todos_los_planes = PrecioSuscripcion.objects.para_pais(pais_usuario).order_by("tipo_plan", "-activo")
...
def demo_uso_api():
    ...
    for plan in planes_inactivos:
        print(f"   {plan.get_pais_display()} - {plan.get_tipo_plan_display()}: {plan.precio_formateado()} (Histórico)")
