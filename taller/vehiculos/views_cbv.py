...
import logging
try:
    from taller.models.marcas_usa import MarcaVehiculo as MarcaVehiculoUSA, ModeloVehiculo as ModeloVehiculoUSA
...
class VehiculoCreateView(LoginRequiredMixin, TenantViewMixin, CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.GET.get('force_us') == '1' and self.request.user.is_staff:
            ...
            log.warning("[FORCE_US] Forzando country=US para usuario=%s (valor real=%s)", self.request.user.username, country)
            country = 'US'
