...
try:
    ...
    if _db_name_check and (_db_host_check.lower() in ("localhost", "127.0.0.1", "::1") or not _db_host_check):
        os.environ["PGSSLMODE"] = "disable"
        os.environ.setdefault("PGSSL", "0")
                    ...
                    if key in conn_params and conn_params[key] is not None:
                        try:
                            if isinstance(conn_params[key], bytes):
                                ...
                                conn_params[key] = conn_params[key].decode('utf-8', errors='replace')
                            ...
                            try:
                                if isinstance(conn_params[key], bytes):
                                    conn_params[key] = conn_params[key].decode('latin-1', errors='replace')
                            except Exception:
