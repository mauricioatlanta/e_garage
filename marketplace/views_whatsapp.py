...
def enviar_whatsapp_cliente(request, documento_id):
    ...
    try:
        ...
        ultimo_envio = WhatsAppEnvio.objects.filter(
            empresa=empresa,
        ...
        if ultimo_envio:
            minutos_transcurridos = int((timezone.now() - ultimo_envio.fecha_envio).total_seconds() / 60)
            return JsonResponse({
                "success": False,
                "error": "rate_limit",
        ...
        link_ver_detalle = request.build_absolute_uri(
            reverse('publico:ver_documento', args=[documento.uuid])
        ) if hasattr(documento, 'uuid') else request.build_absolute_uri(
            f"/portal/documentos/{documento.id}/"
        )
...
def enviar_whatsapp_proveedor(request, casa_repuestos_id, part_number):
    ...
    try:
        empresa = request.user.empresa
        casa_repuestos = get_object_or_404(
            CasaRepuestos, 
            id=casa_repuestos_id, 
            empresa=empresa
        )
        ...
        if not telefono:
            return JsonResponse({
                "success": False,
                "error": "Casa de repuestos no tiene teléfono registrado"
            }, status=400)
