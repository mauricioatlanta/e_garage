    ...
    class Meta:
        ...
        widgets = {
            ...
            "apellido": forms.TextInput(
                attrs={
                    "class": "form-control",
                    "placeholder": _("Last name (optional for companies)"),
                }
            ),
            ...
            "email": forms.EmailInput(
                attrs={"class": "form-control", "placeholder": _("Email (optional)")}
            ),
            "tax_id_type": forms.Select(attrs={"class": "form-control"}),
            "tax_id": forms.TextInput(
                attrs={"class": "form-control", "placeholder": _("Tax ID (required for billing)")}
            ),
            "giro": forms.TextInput(
                attrs={
                    "class": "form-control",
                    "placeholder": _("Business activity / Giro (required for billing)"),
                }
            ),
        }
            ...
            try:
                ...
                try:
                    ...
                    raise forms.ValidationError(
                        {
                            "city": _(
                                "Ciudad no encontrada. Por favor, selecciona una comuna del listado."
                            )
                        }
                    )
            ...
            if self.instance.billing_address:
                ...
                if hasattr(city, "region"):
                    ...
                    estado = Estado.objects.filter(
                        pais=country, nombre__icontains=city.region.nombre
                    ).first()
                    if estado:
                        ciudad_unificada = Ciudad.objects.filter(
                            estado=estado, nombre__iexact=city.nombre
                        ).first()
                        if ciudad_unificada:
