...
con_lineas = (Documento.objects
              .annotate(rep=Count("lineas_repuesto"),
                        ser=Count("lineas_servicio"),
                        otr=Count("lineas_otro_servicio"))
              .filter(rep__gt=0) | Documento.objects
              .annotate(rep=Count("lineas_repuesto"),
                        ser=Count("lineas_servicio"),
                        otr=Count("lineas_otro_servicio"))
              .filter(ser__gt=0) | Documento.objects
              .annotate(rep=Count("lineas_repuesto"),
                        ser=Count("lineas_servicio"),
                        otr=Count("lineas_otro_servicio"))
              .filter(otr__gt=0)).distinct().count()
...
for d in Documento.objects.order_by("-id")[:15]:
    ...
    print(f"   💰 netos= rep:{getattr(d, 'neto_repuestos', 'N/A')} ser:{getattr(d, 'neto_servicios', 'N/A')}")
    print()
