...
def configuracion_tecnicos(request):
    ...
    if request.method == 'POST':
        if 'crear_tecnico' in request.POST:
            ...
            if tecnico:
                ...
                tecnico.save()
                messages.success(request, f'❌ Técnico "{tecnico.nombre}" desactivado. Ya no aparecerá en formularios nuevos.')
            ...
            if tecnico:
                ...
                if documentos_asociados > 0:
                    messages.warning(request, f'⚠️ No se puede eliminar "{tecnico.nombre}" porque tiene {documentos_asociados} documentos asociados. Se ha desactivado en su lugar.')
                    tecnico.activo = False
                    tecnico.save()
                else:
    ...
    return render(request, 'taller/configuracion/tecnicos.html', {
        'tecnicos': tecnicos,
        'empresa': empresa,
        'stats': {
