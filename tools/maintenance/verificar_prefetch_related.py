...
def verificar_prefetch_related():
    ...
    for documento in Documento.objects.all()[:10]:
        ...
        lineas_otro = (
            ...
            if hasattr(documento, "lineas_otro_servicio")
            else 0
        )

        total_lineas = lineas_repuesto + lineas_servicio + lineas_otro
        if total_lineas > 0:
            documentos_con_lineas.append(
                (documento, total_lineas, lineas_repuesto, lineas_servicio, lineas_otro)
            )
            print(
                f"   📄 Doc #{documento.numero_documento}: {total_lineas} líneas (R:{lineas_repuesto}, S:{lineas_servicio}, O:{lineas_otro})"
            )
    ...
    print(
        f"   📊 Resultados: {len(repuestos_sin_prefetch)} repuestos, {len(servicios_sin_prefetch)} servicios, {len(otros_sin_prefetch)} otros"
    )
    ...
    print(
        f"   📊 Resultados: {len(repuestos_con_prefetch)} repuestos, {len(servicios_con_prefetch)} servicios, {len(otros_con_prefetch)} otros"
    )
    ...
    if queries_con_prefetch < queries_sin_prefetch:
        ...
        print(
            f"   📈 Mejora: {mejora}/{queries_sin_prefetch} = {mejora/queries_sin_prefetch*100:.1f}%"
        )
    else:
