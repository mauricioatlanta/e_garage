...
class ProcesadorNotificaciones:
    ...
    def verificar_suscripciones(self):
        ...
        try:
            ...
            if empresas_notificadas:
                self.log_sistema(
                    f"⚠️  {len(empresas_notificadas)} empresas con suscripción por vencer:"
                )
    ...
    def verificar_mantenimientos(self):
        ...
        try:
            ...
            if recordatorios_enviados > 0:
                self.log_sistema(
                    f"🔧 {recordatorios_enviados} recordatorios de mantenimiento enviados"
                )
            else:
    ...
    def generar_reporte_notificaciones(self):
        ...
        try:
            ...
            tipos_populares = (
                NotificacionEnviada.objects.values("tipo_notificacion__nombre")
    ...
    def ejecutar_procesamiento_completo(self):
        ...
        self.log_sistema(
            f"📧 Notificaciones enviadas: {self.estadisticas['notificaciones_enviadas']}"
        )
        self.log_sistema(
            f"🏢 Suscripciones verificadas: {self.estadisticas['suscripciones_verificadas']}"
        )
        self.log_sistema(
            f"🔧 Recordatorios enviados: {self.estadisticas['recordatorios_enviados']}"
        )
