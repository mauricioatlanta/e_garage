...
SUSPECT_PATTERNS = [
    ...
    r"esquema\.sql$",
    r"bom_files\.txt$",
    r"auditor_resultado\.txt$",
    r"django_check_deploy_prod\.txt$",
    r"input\.css$",
    r"moneda\.py$",
    r"context_processors\.py$",
    r"models\.py$",
    r"ia_views\.py$",
    r"onboarding_.*\.py$",
    r"migration_script\.py$",
    r"mark_migration\.py$",
    r"get-pip\.py$",
    r"package-lock\.json$",
    r"package\.json$",
    r"landing_egarage\.html$",
    r"company_settings\.html$",
    r"debug_js\.html$",
    r"diagnostico_imagenes\.html$",
    r"crear_documento_moderno_backup\.html$",
    r"invoice_.*\.pdf$",
    r"\.ps1$",
    r"\.sh$",
]
...
def is_django_app_dir(d: Path) -> bool:
    ...
    hits = sum(
        1
        for h in DJANGO_APP_HINTS
        if h in entries or any(x.name == "__init__.py" for x in d.glob("migrations"))
    )
...
def classify_files(root: Path):
    ...
    for p in root.iterdir():
        ...
        if p.is_dir():
            continue
        name = p.name
        if match_any(name, SUSPECT_PATTERNS):
            suspects.append(p)
        elif match_any(name, DATA_LOADERS):
            loaders.append(p)
        elif match_any(name, NOTES_MD):
            notes.append(p)
        else:
            others.append(p)
