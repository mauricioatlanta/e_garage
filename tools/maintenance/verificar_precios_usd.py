    ...
    def handle(self, *args, **options):
        ...
        if repuestos.exists():
            if hasattr(repuestos.first(), "precio_compra"):
                ...
                self.stdout.write(
                    f'  💰 Compra: ${stats_compra["min_compra"]:.2f} - ${stats_compra["max_compra"]:.2f} (promedio: ${stats_compra["avg_compra"]:.2f})'
                )
            ...
            if hasattr(repuestos.first(), "precio_venta"):
                ...
                self.stdout.write(
                    f'  💰 Venta: ${stats_venta["min_venta"]:.2f} - ${stats_venta["max_venta"]:.2f} (promedio: ${stats_venta["avg_venta"]:.2f})'
                )
        ...
        if servicios.exists():
            if hasattr(servicios.first(), "precio_base"):
                ...
                self.stdout.write(
                    f'  💰 Base: ${stats_servicios["min_precio"]:.2f} - ${stats_servicios["max_precio"]:.2f} (promedio: ${stats_servicios["avg_precio"]:.2f})'
                )
        ...
        if lineas_repuestos.exists():
            ...
            self.stdout.write(
                f'  💰 Precio unitario: ${stats_lineas_rep["min_precio"]:.2f} - ${stats_lineas_rep["max_precio"]:.2f} (promedio: ${stats_lineas_rep["avg_precio"]:.2f})'
            )
            ...
            for linea in lineas_repuestos[:5]:
                self.stdout.write(
                    f"    • {linea.nombre}: ${linea.precio_unitario:.2f} x {linea.cantidad}"
                )
        else:
        ...
        if lineas_servicios.exists():
            ...
            self.stdout.write(
                f'  💰 Precio unitario: ${stats_lineas_serv["min_precio"]:.2f} - ${stats_lineas_serv["max_precio"]:.2f} (promedio: ${stats_lineas_serv["avg_precio"]:.2f})'
            )
        ...
        if lineas_otros.exists():
            ...
            self.stdout.write(
                f'  💰 Costo interno: ${stats_lineas_otros["min_costo"]:.2f} - ${stats_lineas_otros["max_costo"]:.2f} (promedio: ${stats_lineas_otros["avg_costo"]:.2f})'
            )
            self.stdout.write(
                f'  💰 Precio cliente: ${stats_lineas_otros["min_precio"]:.2f} - ${stats_lineas_otros["max_precio"]:.2f} (promedio: ${stats_lineas_otros["avg_precio"]:.2f})'
            )
            ...
            for linea in lineas_otros[:5]:
                self.stdout.write(
                    f"    • {linea.nombre}: costo ${linea.costo_interno:.2f}, precio ${linea.precio_cliente:.2f}"
                )
        else:
