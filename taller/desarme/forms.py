...
ANIOS_CHOICES = [("", "---------")] + [(y, str(y)) for y in range(2026, 1969, -1)]  # 2026-1970 descendente
...
class VehiculoDesarmeForm(forms.ModelForm):
    ...
    transporte_grua = forms.DecimalField(
        ...
        decimal_places=2,
        initial=None,
        widget=forms.NumberInput(attrs={"class": "input-desarme", "step": "0.01", "placeholder": "0"}),
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        if "motor" in self.fields:
            ...
            self.fields["motor"].queryset = MotorVehiculo.objects.filter(country=pais).order_by("nombre")

        # Caja: Select nativo
        if "caja" in self.fields:
            self.fields["caja"].widget = forms.Select(attrs={"class": "input-desarme"})
            self.fields["caja"].queryset = CajaVehiculo.objects.filter(country=pais).order_by("nombre")
        ...
        if "vendedor_desarme" in self.fields:
            ...
            if self.empresa:
                self.fields["vendedor_desarme"].queryset = (
                    VendedorDesarme.objects.filter(empresa=self.empresa).order_by("nombre")
                )
        ...
        if "observaciones_desarme" in self.fields:
            self.fields["observaciones_desarme"].widget.attrs.update({"rows": 2, "class": "input-desarme"})
        # Estilo input-desarme en todos los campos
        for field in self.fields.values():
            if hasattr(field.widget, "attrs") and "input-desarme" not in (field.widget.attrs.get("class") or ""):
        ...
        if self.empresa:
            ...
            if patente:
                ...
                if self.instance.pk:
                    qs = qs.exclude(pk=self.instance.pk)
                if qs.exists():
                    self.add_error("patente", "Ya existe un vehículo con esta patente en esta empresa.")
    ...
    def save(self, commit=True):
        ...
        if extras and instance.observaciones_desarme:
            instance.observaciones_desarme = instance.observaciones_desarme.rstrip() + "\n" + "\n".join(extras)
        elif extras:
...
class PiezaDesarmeForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for f in ("fecha_extraccion", "ubicacion_fisica", "observaciones", "lado", "zona", "posicion"):
            if f in self.fields:
                self.fields[f].required = False
