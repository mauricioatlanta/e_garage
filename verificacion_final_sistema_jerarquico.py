...
if __name__ == "__main__":
    ...
    if modelos_data:
        ...
        if response.status_code == 200:
            ...
            print(
                f"   📡 load-motores-cajas: {response.status_code} (motores: {len(combinado_data.get('motores', []))}, cajas: {len(combinado_data.get('cajas', []))})"
            )
        else:
        ...
        if modelos.exists():
            for modelo in modelos[:2]:  # Revisar primeros 2 modelos por marca
                ...
                if motores.exists() and cajas.exists():
                    print(
                        f"   ✅ {marca.nombre} → {modelo.nombre}: {motores.count()} motores, {cajas.count()} cajas"
                    )
    ...
    all_endpoints_ok = all(
        [
            ...
            for response in [
                ...
                client.get(
                    f"/cl/taller/ajax/load-motores/?modelo_id={primer_modelo_id}"
                    if "primer_modelo_id" in locals()
                    else "/cl/taller/ajax/load-motores/?modelo_id=1"
                ),
                client.get(
                    f"/cl/taller/ajax/load-cajas/?modelo_id={primer_modelo_id}"
                    if "primer_modelo_id" in locals()
                    else "/cl/taller/ajax/load-cajas/?modelo_id=1"
                ),
            ]
        ]
    )
