...
def seed_documentos():
    ...
    print(f"✅ Clientes: {c1.nombre} {c1.apellido}, {c2.nombre} {c2.apellido}, {c3.nombre} {c3.apellido}")
    ...
    serv_cambio, _ = Servicio.objects.get_or_create(
        empresa=emp, 
        nombre='Cambio de Aceite Estándar', 
        defaults={'precio':25000}
    )
    ...
    docs = []
    docs.append(crear_doc(c1, v1, 'FAC', rep_lines=[('r002',5,19000), ('r005',2,12000)], serv_lines=[(serv_cambio,1,25000)], km=15000))
    docs.append(crear_doc(c2, v2, 'FAC', rep_lines=[('r001',1,15000)], serv_lines=[(serv_frenos,1,45000)], km=25000))
    docs.append(crear_doc(c3, v3, 'OT',  rep_lines=[('r003',4,6000)],  serv_lines=[(serv_cambio,1,25000)], km=8000))
    docs.append(crear_doc(c1, v1, 'PRES',rep_lines=[('r004',1,32000)], serv_lines=[], km=16000))
    docs.append(crear_doc(c2, v2, 'OT',  rep_lines=[('r001',2,15000),('r005',1,12000)], serv_lines=[], km=30000))
    
    print("✅ DOCUMENTOS CREADOS:")
    for doc in docs:
        print(f"   {doc.numero} - {doc.tipo} - {doc.cliente.nombre} {doc.cliente.apellido} - ${doc.total:,.0f}")
