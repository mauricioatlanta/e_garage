...
def api_buscar_repuestos(request):
    ...
    repuestos = Repuesto.objects.filter(
        ...
        Q(nombre__icontains=query) | Q(part_number__icontains=query)
    ).values('id', 'part_number', 'nombre', 'precio_compra', 'precio_venta', 'cantidad_stock', 'proveedor')[:20]
...
from taller.servicios.models import Servicio as ServicioInterno, ServicioExterno
...
def obtener_datos_formulario(empresa):
    ...
    servicios_externos = ServicioExterno.objects.filter(empresa=empresa, activo=True).select_related('categoria')
...
def procesar_documento_moderno(request, empresa):
    ...
    try:
        ...
        kilometraje = request.POST.get('kilometraje') or request.POST.get('millas')  # millas/odómetro
        ...
        documento = Documento.objects.create(
            empresa=empresa,
            tipo=tipo,
            fecha_emision=datetime.strptime(str(fecha_emision), '%Y-%m-%d').date() if isinstance(fecha_emision, str) else fecha_emision,
            cliente=cliente,
            ...
            **({"kilometraje": kilometraje} if hasattr(Documento, "kilometraje") and kilometraje else {})
        )

        # Generar número de documento
        ultimo_doc = Documento.objects.filter(empresa=empresa, tipo=tipo).order_by('-numero').first()
        proximo_numero = (ultimo_doc.numero + 1) if ultimo_doc and ultimo_doc.numero else 1
        ...
        try:
            ...
            rep_ids = request.POST.getlist("repuestos_ids[]") or request.POST.getlist("repuesto_id[]")
            rep_parts = request.POST.getlist("repuestos_partnumber[]") or request.POST.getlist("repuesto_partnumber[]")
            ...
            print("DEBUG rep_ids:", len(rep_ids or []),
                  "rep_parts:", len(rep_parts or []),
                  "cant:", len(rep_cant or []),
                  "precio:", len(rep_precio or []),
                  "desc:", len(rep_desc or []))
            
            repuestos_data = []
            total_items = max(len(rep_ids or []), len(rep_parts or []), len(rep_cant or []), len(rep_precio or []), len(rep_desc or []))
            for i in range(total_items):
                repuesto_id = (rep_ids[i] if rep_ids and len(rep_ids) > i and rep_ids[i] else None)
                partnum = (rep_parts[i] if rep_parts and len(rep_parts) > i and rep_parts[i] else None)
                if not repuesto_id and partnum:
                    rep_obj = Repuesto.objects.filter(part_number__iexact=partnum, empresa=empresa).first()
                    repuesto_id = rep_obj.id if rep_obj else None
                    print(f"DEBUG: Resolviendo part_number '{partnum}' -> repuesto_id={repuesto_id}")
                if not repuesto_id:
        ...
        if hasattr(empresa, 'configuracion') and hasattr(empresa.configuracion, 'dividir_por_tecnico'):
        ...
        for item in otros_servicios_data:
            ...
            if hasattr(item, 'get') and item.get('id'):
                ...
                servicio_externo = get_object_or_404(ServicioExterno, id=item['id'], empresa=empresa)
            ...
            linea_otro_servicio = LineaOtroServicio.objects.create(
                documento=documento,
                servicio_externo=servicio_externo if 'servicio_externo' in locals() and item.get('id') else None,
                nombre=nombre,
        ...
        rep_subtotal = sum([
            lr.cantidad * lr.precio_unitario * (Decimal("1.0") - (lr.descuento or Decimal(0))/Decimal("100")) 
            for lr in documento.lineas_repuesto.all()
        ])
        serv_subtotal = sum([
            ls.cantidad * ls.precio_unitario * (Decimal("1.0") - (ls.descuento or Decimal(0))/Decimal("100")) 
        ...
        if incluir_impuesto:
            if empresa.pais == 'CL':
                ...
                tax_amount = (total_gravable * tax_rate_usa / Decimal("100")).quantize(Decimal("0.01"))
                tax_rate_applied = tax_rate_usa

        total = (rep_subtotal + serv_subtotal + otro_subtotal + tax_amount).quantize(Decimal("0.01"))
        ...
        documento.save(update_fields=["neto_repuestos", "neto_servicios", "tax_rate_applied", "tax_amount", "total"])

        messages.success(request, f"Documento {documento.tipo}-{documento.numero} creado exitosamente")
...
def api_obtener_numero_documento(request):
    ...
    try:
        ...
        tipo_documento_normalizado = tipo_mapping.get(tipo_documento.upper(), tipo_documento.upper())
