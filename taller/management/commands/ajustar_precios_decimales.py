...
from taller.models import (
    Empresa, Repuesto, LineaRepuesto, LineaServicio, LineaOtroServicio
)
...
def generar_precio_con_decimales(base_precio, variacion=0.15):
    ...
    decimales_comunes = [0.99, 0.95, 0.89, 0.79, 0.69, 0.59, 0.49, 0.39, 0.29, 0.19, 0.09]
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for repuesto in Repuesto.objects.filter(empresa=empresa):
            ...
            for nombre_patron, (min_precio, max_precio) in precios_base_repuestos.items():
            ...
            if hasattr(repuesto, 'precio_compra') and repuesto.precio_compra != precio_compra:
                ...
                self.stdout.write(f'  💰 {repuesto.nombre}: compra ${repuesto.precio_compra} → ${precio_compra}')
            ...
            if hasattr(repuesto, 'precio_venta') and repuesto.precio_venta != precio_venta:
                ...
                self.stdout.write(f'  💰 {repuesto.nombre}: venta ${repuesto.precio_venta} → ${precio_venta}')
                repuestos_ajustados += 1
        ...
        for servicio in Servicio.objects.filter(empresa=empresa):
            if hasattr(servicio, 'precio_base'):
                # Buscar precio baseado en el nombre
                precio_base = None
                for nombre_patron, (min_precio, max_precio) in precios_base_servicios.items():
                    if nombre_patron.lower() in servicio.nombre.lower():
                        precio_base = generar_precio_con_decimales(RND.uniform(min_precio, max_precio))
                        break
                ...
                if servicio.precio_base != precio_base:
                    ...
                    self.stdout.write(f'  💰 {servicio.nombre}: ${servicio.precio_base} → ${precio_base}')
        ...
        for linea in LineaRepuesto.objects.filter(documento__empresa=empresa):
            ...
            if linea.precio_unitario != nuevo_precio:
                ...
                self.stdout.write(f'  💰 Línea repuesto {linea.nombre}: ${linea.precio_unitario} → ${nuevo_precio}')
        ...
        for linea in LineaServicio.objects.filter(documento__empresa=empresa):
            ...
            if linea.precio_unitario != nuevo_precio:
                if not dry_run:
                    linea.precio_unitario = nuevo_precio
                self.stdout.write(f'  💰 Línea servicio {linea.nombre}: ${linea.precio_unitario} → ${nuevo_precio}')
        ...
        for linea in LineaOtroServicio.objects.filter(documento__empresa=empresa):
            ...
            if linea.costo_interno != nuevo_costo:
                ...
                self.stdout.write(f'  💰 {linea.nombre}: costo ${linea.costo_interno} → ${nuevo_costo}')
                lineas_otros_ajustadas += 1
            
            if linea.precio_cliente != nuevo_precio:
                if not dry_run:
                    linea.precio_cliente = nuevo_precio
                self.stdout.write(f'  💰 {linea.nombre}: precio ${linea.precio_cliente} → ${nuevo_precio}')
                lineas_otros_ajustadas += 1
        ...
        # ==========================================
        total_ajustados = (
            repuestos_ajustados + servicios_ajustados + 
            lineas_repuestos_ajustadas + lineas_servicios_ajustadas + 
        ...
        if dry_run:
            self.stdout.write(
                self.style.WARNING(f'🔍 MODO SIMULACIÓN: Se habrían ajustado {total_ajustados} precios con decimales')
            )
        else:
            self.stdout.write(
                self.style.SUCCESS(f'✅ AJUSTE COMPLETADO: {total_ajustados} precios ajustados con decimales')
            )
