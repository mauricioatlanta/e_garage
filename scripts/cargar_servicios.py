...
# Crear categorías y subcategorías de ejemplo
data = {
    "Mecánica General": [
...
for cat_nombre, subcats in data.items():
    categoria, _ = CategoriaServicio.objects.get_or_create(nombre=cat_nombre)
    for sub_nombre in subcats:
        subcat, _ = SubcategoriaServicio.objects.get_or_create(categoria=categoria, nombre=sub_nombre)
        Servicio.objects.get_or_create(nombre=sub_nombre, subcategoria=subcat)
