    ...
    if csrf_match:
        ...
        if login_response.status_code in [200, 302]:
            ...
            if crear_doc.status_code == 200:
                ...
                csrf_form = re.search(
                    r'name="csrfmiddlewaretoken" value="([^"]*)"', crear_doc.text
                )
                if csrf_form:
                    ...
                    docs_after = Documento.objects.filter(
                    ...
                    if docs_after.exists():
                        ...
                        print(
                            f"   ✅ Último doc: {ultimo_doc.pk} - R:{repuestos.count()} S:{servicios.count()}"
                        )
                    else:
...
for doc in docs_taller2:
    ...
    print(
        f"   Doc {doc.pk}: {doc.trabajo_solicitado[:20]} - R:{repuestos.count()} S:{servicios.count()}"
    )
