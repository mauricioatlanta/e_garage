...
from taller.middleware.lang_policy import LanguagePolicyMiddleware
import re
...
def test_usa_registration():
    ...
    results.append(("Términos en inglés (Monthly)", has_english_month, f"Encontrado: {has_english_month}"))
    
    # Verificar que hay traducción (tags {% trans %})
    has_i18n = '{% trans' in open('templates/account/signup.html', 'r', encoding='utf-8').read()
    results.append(("Template usa i18n ({% trans %})", has_i18n, f"Template con soporte multiidioma"))
    ...
    results.append(("USA permite inglés y español", ('en' in usa_allowed and 'es' in usa_allowed), 
...
def test_chile_registration():
    ...
    results.append(("Chile solo permite español", chile_allowed == ('es',), 
    ...
    results.append(("Chile NO permite cambio a inglés", 'en' not in chile_allowed, 
...
def test_templates_exist():
    ...
    results.append((f"Template signup principal existe", exists, signup_template))
    
    # Template alternativo
    signup_auth = 'templates/auth/signup.html'
    exists_auth = os.path.exists(signup_auth)
    results.append((f"Template signup auth existe", exists_auth, signup_auth))
    ...
    results.append((f"Template bienvenida Chile existe", exists_cl, bienvenida_chile))
    
    # Template de bienvenida USA
    bienvenida_usa = 'templates/onboarding/bienvenida_usa.html'
    exists_us = os.path.exists(bienvenida_usa)
    results.append((f"Template bienvenida USA existe", exists_us, bienvenida_usa))
    
    # Verificar contenido de bienvenida Chile (debe estar en español)
    if exists_cl:
        content = open(bienvenida_chile, 'r', encoding='utf-8').read()
        has_spanish_content = 'Chile' in content and ('Taller' in content or 'Digitaliza' in content)
        lang_es = 'lang="es"' in content
        results.append(("Bienvenida Chile en español", has_spanish_content and lang_es, 
                       "Contenido verificado en español"))
    
    # Verificar contenido de bienvenida USA (debe tener i18n)
    if exists_us:
        content = open(bienvenida_usa, 'r', encoding='utf-8').read()
        has_i18n = '{% load i18n %}' in content and '{% trans' in content
        has_lang_var = 'LANGUAGE_CODE' in content
        results.append(("Bienvenida USA con i18n", has_i18n or has_lang_var, 
...
def test_middleware_configuration():
    ...
    results.append(("Middleware de idiomas configurado", has_lang_policy or has_country, 
    ...
    results.append(("Inglés configurado", has_english, f"Idiomas: {[code for code, _ in languages]}"))
    results.append(("Español configurado", has_spanish, f"Idiomas: {[code for code, _ in languages]}"))
...
def main():
    ...
    print(f"{Colors.BOLD}{Colors.BLUE}{'TEST DE REGISTRO POR PAÍS - E_GARAGE'.center(70)}{Colors.END}")
