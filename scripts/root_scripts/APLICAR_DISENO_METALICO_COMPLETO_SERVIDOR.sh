#!/bin/bash
# Script para aplicar el nuevo diseño metálico futurista
# - Sin header
# - Logo imponente centrado
# - Países como chapitas metálicas con colores de banderas
# - Fondo animado super futurista (negro, calipso, neon)

cd /home/atlantareciclajes/apps/egarage/current && \
source ~/.virtualenvs/venv_egarage310/bin/activate && \

echo "🔧 Aplicando nuevo diseño metálico futurista..."

# Backup
cp templates/public/selector_pais.html templates/public/selector_pais.html.backup_$(date +%Y%m%d_%H%M%S)

# Copiar archivo completo usando Python para manejar caracteres especiales
python3 << 'PYEOF'
file_path = "templates/public/selector_pais.html"

# Leer el contenido completo del archivo local
# Nota: Este script asume que el archivo local ya tiene el contenido correcto
# Si estás ejecutando esto desde el servidor, necesitarás copiar el archivo de otra manera

# Por ahora, escribimos el contenido completo directamente
content = """{% load static %}
<!DOCTYPE html>
<html lang="es">
<head>
  ...
  <style>
    * { 
    ...
    /* Tarjetas de país - CHAPITAS METÁLICAS */
    .country-card {
      position: relative;
      display: flex;
    ...
    .country-card:hover .country-flag {
      ...
      filter: drop-shadow(0 10px 30px rgba(0, 255, 255, 0.8));
    }
    ...
    .country-code {
      ...
      text-shadow: 0 0 20px rgba(0, 255, 255, 0.6), 0 4px 10px rgba(0, 0, 0, 0.8);
      transition: all 0.3s;
    }

    .country-card:hover .country-code {
      text-shadow: 0 0 30px rgba(0, 255, 255, 1), 0 0 50px rgba(0, 191, 255, 0.8);
      letter-spacing: 8px;
    }
    ...
    .country-card:hover .country-name {
      text-shadow: 0 0 40px rgba(255, 255, 255, 1), 0 0 60px rgba(0, 255, 255, 0.6);
    }

    /* Estilos específicos por país - Colores de banderas bien definidos */
    .country-card.usa {
      background: linear-gradient(135deg, #002868 0%, #1a3a8a 30%, #002868 60%, #BF0A30 100%);
      border-color: rgba(191, 10, 48, 0.4);
    }

    .country-card.brasil {
      background: linear-gradient(135deg, #009c3b 0%, #00a84f 25%, #ffdf00 50%, #ffcc00 75%, #009c3b 100%);
...
echo ""
echo "✅✅✅ Diseño metálico futurista aplicado ✅✅✅"
echo ""
