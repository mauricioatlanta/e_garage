...
def check_1_modelo_y_datos():
    ...
    print(
        f"📁 Archivo del modelo: e:\\projecto\\e_garage\\taller\\models\\configuracion.py"
    )
...
def check_2_media_settings():
    ...
    if os.path.exists(settings.MEDIA_ROOT):
        ...
        if os.path.exists(logos_path):
            print(f"✅ Carpeta logos/ existe")
            files = os.listdir(logos_path)
            print(f"📁 Archivos: {files}")
        else:
            print(f"❌ Carpeta logos/ NO existe")
    else:
...
def check_4_template_variables():
    ...
    if os.path.exists(base_template_path):
        ...
        print(f"🔍 Buscando variables de branding en template:")
        ...
        print(f"❌ Base template NO encontrado")
    ...
    for template_path in pdf_template_paths:
        ...
        if os.path.exists(full_path):
            ...
            if "build_absolute_uri" in content:
                print(f"   ✅ Usa build_absolute_uri")
            else:
                print(f"   ❌ NO usa build_absolute_uri")

            # Buscar variables de logo
            if "company_logo" in content:
                print(f"   ✅ Usa company_logo")
            else:
                print(f"   ❌ NO usa company_logo")
        else:
